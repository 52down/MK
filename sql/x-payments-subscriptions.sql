...
CREATE TABLE `xcart_xps_products` (
  ...
  `fee` decimal(12,2) NOT NULL COMMENT 'Use func_decimal_empty',
...
CREATE TABLE `xcart_xps_subscriptions` (
  ...
  `fee` decimal(12,2) NOT NULL DEFAULT '0.00' COMMENT 'Use func_decimal_empty',
...
REPLACE INTO `xcart_config` (`name`, `comment`, `value`, `category`, `orderby`, `type`, `defvalue`, `variants`, `validation`, `signature`) VALUES('xps_start_subscription_on_auth', 'Activate subscription if payment is made in "Auth only" mode', 'Y', 'XPayments_Subscriptions', 40, 'checkbox', 'Y', '', '', '');
