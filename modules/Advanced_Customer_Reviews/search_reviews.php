 ...
 * @version    cf9e608d41c40f761c6416f642a1d0094a6af214, v18 (xcart_4_7_7), 2017-01-24 09:29:34, search_reviews.php, aim
        ...
        if ($mode == 'acr_update_reviews') {
            ...
            if (is_array($posted)) {
                foreach ($posted as $review_id => $review) {
                    ...
                    if (empty($review['status']) || $review['status'] == 'A') {
                        func_acr_handle_discount_coupon($review_id);
                    }
                }
...
if ($mode == 'search') {
    ...
    if ($total_items > 0) {
        ...
        if (
            defined('XC_SESSION_DB_SAVE_SEARCH_REVIEWS')
            && $flag_save
        ) {
