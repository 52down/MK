 ...
 * @version    cf9e608d41c40f761c6416f642a1d0094a6af214, v24 (xcart_4_7_7), 2017-01-24 09:29:34, func.php, aim
...
function func_acr_send_review_reminder($userinfo, $products, $test_email = '') {
    ...
    if (!empty($products)) {
        ...
        if ($config['Advanced_Customer_Reviews']['acr_reward_coupon_discount'] > 0) {

            $_discount_coupon_value = ($config['Advanced_Customer_Reviews']['acr_reward_coupon_type'] == 'P') ?
                ($config['Advanced_Customer_Reviews']['acr_reward_coupon_discount'] . '%') :
                ($config['General']['currency_symbol'] . $config['Advanced_Customer_Reviews']['acr_reward_coupon_discount']);
            $mail_smarty->assign('discount_coupon', $_discount_coupon_value);

            $result = func_send_mail($email, 'mail/acr_review_reminder_coupon_subj.tpl', 'mail/acr_review_reminder.tpl', $config['Company']['site_administrator'], false);
        } else {
...
function func_acr_handle_discount_coupon($review_id) {//{{{
    global $sql_tbl, $active_modules, $mail_smarty, $config, $single_mode;

    // turned off
    if ($config['Advanced_Customer_Reviews']['acr_reward_coupon_discount'] <= 0 || empty($active_modules['Discount_Coupons'])) {
        return false;
    }
    $review_id = intval($review_id);

    $data = func_query_first("SELECT * FROM $sql_tbl[product_reviews] WHERE review_id = '$review_id'");

    // review is not approved or coupon is already sent or email is empty
    if (empty($data) || $data['status'] != 'A' || $data['is_coupon_sent'] == 'Y' || empty($data['email'])) {
        return false;
    }

    // generate discount coupon and send it
    for ($i = 0; $i < 10; $i++) {
        $coupon = substr(md5(uniqid()), 0, 6);
        if (!func_query_first_cell("SELECT coupon FROM $sql_tbl[discount_coupons] WHERE coupon = '$coupon'")) {
            break;
        }
    }

    $coupon_type_new = $config['Advanced_Customer_Reviews']['acr_reward_coupon_type'] == 'P' ? 'percent' : 'absolute';

    // use product provider in pro
    $prov_userid = ($single_mode) ? 0 : func_query_first_cell("SELECT provider FROM $sql_tbl[products] WHERE productid = '$data[productid]'");

    $coupon_data = array(
        'coupon'              => $coupon,
        'discount'            => $config['Advanced_Customer_Reviews']['acr_reward_coupon_discount'],
        'coupon_type'         => $coupon_type_new,
        'minimum'             => 0,
        'times'               => 1,
        'per_user'            => 'N',
        'expire'              => XC_TIME + 365 * SECONDS_PER_DAY, // 1 year
        'status'              => 'A',
        'provider'            => $prov_userid,
        'productid'           => 0,
        'categoryid'          => 0,
        'recursive'           => 'N',
    ...
    $mail_smarty->assign('coupon_code', $coupon);
    $mail_smarty->assign('fullname', $data['author']);

    func_send_mail($data['email'], 'mail/acr_your_coupon_subj.tpl', 'mail/acr_your_coupon.tpl', $config['Company']['site_administrator'], false);

    db_query("UPDATE $sql_tbl[product_reviews] SET is_coupon_sent = 'Y' WHERE review_id = '$review_id'");

    return true;
}//Func_acr_handle_discount_coupon}}}
