 ...
 * @version    cf9e608d41c40f761c6416f642a1d0094a6af214, v97 (xcart_4_7_7), 2017-01-24 09:29:34, returns.php, aim
...
if (!empty($search['date_range'])) {
    ...
    $parsed_daterange = func_daterange_get_start_end_timestamp_from_range($search['date_range']);

    if (!empty($parsed_daterange)) {
        $search_data['returns']['start_date'] = $parsed_daterange['start_timestamp'];
        $search_data['returns']['end_date']   = $parsed_daterange['end_timestamp'];
    }
}
 ...
 */
    ...
    if (!empty($active_modules['AvaTax'])) {
        if ($old_status != 'A' && $old_status != 'C' && ($posted_data['status'] == 'A' || $posted_data['status'] == 'C')) {
            func_avatax_return($returnid);
        }
    }
 ...
 */
    ...
    foreach($update as $k => $v) {
        ...
        if (!empty($active_modules['AvaTax'])) {
            if ($old_status != 'A' && $old_status != 'C' && ($v['status'] == 'A' || $v['status'] == 'C')) {
                func_avatax_return($k);
            }
        }
...
if ($mode == 'modify') {
    ...
    if ($current_area == 'C') {
        ...
        if (!empty($access_key) && empty($login)) {
            $where[] = "$sql_tbl[orders].access_key = '$access_key'";
	}
    }
    ...
    $returns = func_query("SELECT $sql_tbl[returns].*, IFNULL($sql_tbl[products_lng_current].product, 'PRODUCT (deleted from database)') as product, IFNULL($sql_tbl[products_lng_current].productid, 0) as productid, $sql_tbl[orders].orderid, $sql_tbl[orders].firstname, $sql_tbl[orders].lastname, $sql_tbl[orders].userid, $sql_tbl[orders].all_userid, $sql_tbl[orders].date as order_date, $sql_tbl[order_details].amount as order_amount, $sql_tbl[order_details].price, $sql_tbl[order_details].product_options, $sql_tbl[order_details].extra_data FROM $sql_tbl[returns], $sql_tbl[orders], $sql_tbl[order_details] LEFT JOIN $sql_tbl[products_lng_current] ON $sql_tbl[order_details].productid = $sql_tbl[products_lng_current].productid WHERE $sql_tbl[returns].itemid = $sql_tbl[order_details].itemid AND $sql_tbl[orders].orderid = $sql_tbl[order_details].orderid".$where.$sort_string);

    if (!empty($returns)) {
        foreach ($returns as $k => $v) {
            $return_extra_data = unserialize($v['extra_data']);
            $total_product_taxes = 0;
            foreach ($return_extra_data['taxes'] as $kk => $vv) {
                $total_product_taxes += $vv['tax_value'];
            }
            $returns[$k]['price'] += $total_product_taxes;
            unset($returns[$k]['extra_data'], $return_extra_data, $total_product_prices);
        }
    }
