 ...
 * http://www.authorize.net/content/dam/authorize/documents/SIM_guide.pdf http://www.authorize.net/support/SIM_guide.pdf
 *
 * @category   X-Cart
 * @package    X-Cart
 ...
 * @version    cf9e608d41c40f761c6416f642a1d0094a6af214, v77 (xcart_4_7_7), 2017-01-24 09:29:34, cc_authorizenet_sim.php, aim
 * @link       http://www.x-cart.com/
 * @see        ____file_see____
 *
 */

if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['x_response_code'])) {

    require __DIR__.'/auth.php';

    if (defined('XC_AUTHORIZENET_DEBUG')) {
        func_pp_debug_log('authorizenet-sim', 'R', $_POST);
    }
    ...
    if (!empty($x_auth_code)) {
        $bill_output['billmes'].= " (AuthCode: " . $x_auth_code . ") ";
    }
    if (!empty($x_trans_id)) {
        $bill_output['billmes'].= " (TransID: " . $x_trans_id . ") ";
    }
    if (!empty($x_response_subcode)) {
        $bill_output['billmes'].= " (subcode/reasoncode: " . $x_response_subcode . '/' . $x_response_reason_code . ") ";
    }
    if (!empty($x_avs_code)) {
        $bill_output['avsmes'] = ($avserr[$x_avs_code] ? $avserr[$x_avs_code] : "AVS Code: " . $x_avs_code);
    }
    if (!empty($x_CVV2_Resp_Code)) {
        $bill_output['avsmes'] = ($cvverr[$x_CVV2_Resp_Code] ? $cvverr[$x_CVV2_Resp_Code] : "CVV Code: " . $x_CVV2_Resp_Code);
    }
    ...
    $currencycode    = $module_params['param05'] == 'used_from_authorize' ? '' : $module_params['param05'];
    ...
    if (!$follow_on) {
        ...
        $fp_hash = hmac($txnkey, $loginid . "^" . $sequence . "^" . $tstamp . "^" . $cart['total_cost'] . "^" . $currencycode);

        // Used in: x_invoice_num, 20-character maximum ( http://www.authorize.net/support/SIM_guide.pdf )
        $REF = substr($module_params['param04'] . join("-", $secure_oid), 0, 20);

        if (!$duplicate) {
            db_query("REPLACE INTO $sql_tbl[cc_pp3_data] (ref,sessid) VALUES ('".addslashes($REF)."','".$XCARTSESSID."')");
        }
        ...
        $fields = array(
            ...
            'x_relay_url'           => ($module_params['testmode'] == 'Y' ? $http_location : $current_location) . '/payment/cc_authorizenet_sim.php',
            'x_customer_ip'         => func_get_valid_ip($REMOTE_ADDR),
            'x_solution_id'         => $module_params['testmode'] == 'Y' ? 'AAA100302' : 'AAA105360',
        );

        if (!empty($currencycode)) {
            $fields['x_currency_code'] = $currencycode;
        }
    ...
    if (defined('XC_AUTHORIZENET_DEBUG')) {
        func_pp_debug_log('authorizenet-sim', 'I', $fields);
    }
