{*
065a4f72a8f0cacf4511b0c2cb9edd4d716f53fa, v10 (xcart_4_7_4), 2015-09-30 12:03:07, order_invoice.tpl, random
...
<table class="block-grid two-up address-table-container" summary="{$lng.lbl_addresses|escape}">
  <tr>
    ...
    </td>{if not $active_modules.POS_System or not $pos_hide_shipping_info}<td>
...
{if $aom_changes_comment ne ""}
  {capture name="row"}
    <p style="font-size: 14px; font-weight: bold; text-align: center;">{$lng.lbl_aom_comment}</p>
    <div style="border: 1px solid #cecfce; padding: 5px;">{$aom_changes_comment|nl2br}</div>
  {/capture}
  {include file="mail/html/responsive_row.tpl" content=$smarty.capture.row}
{/if}
