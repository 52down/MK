{*
065a4f72a8f0cacf4511b0c2cb9edd4d716f53fa, v14 (xcart_4_7_4), 2015-09-30 12:03:07, order_invoice.tpl, random
...
<table cellspacing="0" cellpadding="0" {if $is_nomail eq 'Y'}class="order-invoice width-100"{else} style="width:600px; background-color: #ffffff;"{/if} summary="{$lng.lbl_order_details_label|escape}">
  <tr>
    <td>
      ...
      {* if $active_modules.XOrder_Statuses}
        {include file="modules/XOrder_Statuses/order_progress_bar.tpl" order=$order}
      {/if *}
  ...
  {if $aom_changes_comment ne ""}
    <tr>
      <td colspan="3" {if $is_nomail eq 'Y'}class="invoice-customer-notes"{else}style="padding-top: 30px;"{/if}>
        <p{if $is_nomail ne 'Y'} style="font-size: 14px; font-weight: bold; text-align: center;"{/if}>{$lng.lbl_aom_comment}</p>
        <div{if $is_nomail ne 'Y'} style="border: 1px solid #cecfce; padding: 5px;"{/if}>{$aom_changes_comment|nl2br}</div>
      </td>
    </tr>
  {/if}
