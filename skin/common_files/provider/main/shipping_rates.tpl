{*
19a345dbf6556c4e5473a748b716b2affdf148af, v5 (xcart_4_7_7), 2016-12-14 12:32:23, shipping_rates.tpl, mixon
...
{capture name='txt_percent_charge_help'}
<b>DST</b> - {$lng.lbl_discounted_subtotal|wm_remove|escape}<br>
<b>ST</b>  - {$lng.lbl_subtotal|wm_remove|escape}<br>
<b>RTR</b> - {$lng.lbl_realtime_rate|wm_remove|escape}<br>
{/capture}
...
<script type="text/javascript" language="JavaScript 1.2">
//<![CDATA[
checkboxes{$zones_list[zone].zone.zoneid}_{$shipid} = new Array({section name=rate loop=$shipping_method.rates}{if not $smarty.section.rate.first},{/if}'posted_data[{$shipping_method.rates[rate].rateid}][to_delete]'{/section});
//]]>  
</script> 
...
<input type="text" name="posted_data[{$shipping_rate.rateid}][maxweight]" size="9" value="{$shipping_rate.maxweight|formatprice}" />
  ...
  <td>
    {if $type eq 'R'}
    <select name="posted_data[{$shipping_rate.rateid}][rate_p_apply_to]">
    <option value="DST"{if $shipping_rate.rate_p_apply_to eq "DST" or $shipping_rate.rate_p_apply_to eq ""} selected="selected"{/if}>DST</option>
    <option value="ST"{if $shipping_rate.rate_p_apply_to eq "ST"} selected="selected"{/if}>ST</option>
    <option value="RTR"{if $shipping_rate.rate_p_apply_to eq "RTR"} selected="selected"{/if}>RTR</option>
    </select>
    {include file="main/tooltip_js.tpl" text=$smarty.capture.txt_percent_charge_help type='img' id="rate_p_apply_to_{$shipping_rate.rateid}"}
    {/if}
    <input type="text" name="posted_data[{$shipping_rate.rateid}][rate_p]" size="5" value="{$shipping_rate.rate_p|formatprice}" />
  </td>
</tr>

<tr>
  <td>{$lng.lbl_order_range}:</td>
  <td nowrap="nowrap">
...
{if not $smarty.section.rate.last}
<tr>
...
<tr>
  <td><b>{$lng.lbl_order_range_base}:</b></td>
  <td>&nbsp;</td>
  <td>
...
<input type="text" name="maxweight_new" size="9" value="{$maxvalue|formatprice}" />
  ...
  <td>
    {if $type eq 'R'}
    <select name="rate_p_apply_to_new">
    <option value="DST" selected="selected">DST</option>
    <option value="ST">ST</option>
    <option value="RTR">RTR</option>
    </select>
    {include file="main/tooltip_js.tpl" text=$smarty.capture.txt_percent_charge_help type='img' id="rate_p_apply_to_new"}
    {/if}
    <input type="text" name="rate_p_new" size="5" value="{0|formatprice}" />
  </td>
</tr>

<tr>
  <td><b>{$lng.lbl_order_range}:</b></td>
  <td nowrap="nowrap">
