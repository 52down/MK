{*
194626627da00f55901e393daa481ee1a5d33cfb, v9 (xcart_4_7_4), 2015-10-26 13:19:01, register_personal_info.tpl, aim
...
{if $default_fields.firstname.avail eq 'Y'}
      <tr>
        ...
        <td>
          <input type="text" id="firstname" name="firstname" size="32" maxlength="128" value="{$userinfo.firstname|escape}" {$autofocus} />
...
{if $default_fields.lastname.avail eq 'Y'}
      <tr>
        ...
        <td>
          <input type="text" id="lastname" name="lastname" size="32" maxlength="128" value="{$userinfo.lastname|escape}" {$autofocus} />
          {assign var=autofocus value=''}
        </td>
      </tr>
{/if}
{if $default_fields.company.avail eq 'Y'}
      <tr>
        ...
        <td>
          <input type="text" id="company" name="company" size="32" maxlength="255" value="{$userinfo.company|escape}" {$autofocus} />
          {assign var=autofocus value=''}
        </td>
      </tr>
{/if}
{if $default_fields.url.avail eq 'Y'}
      <tr>
        ...
        <td>
          <input type="text" id="url" name="url" size="32" maxlength="128" value="{$userinfo.url|escape}" {$autofocus} />
          {assign var=autofocus value=''}
        </td>
      </tr>
{/if}
{if $default_fields.ssn.avail eq 'Y'}
      <tr>
        ...
        <td>
          <input type="text" id="ssn" name="ssn" size="32" maxlength="32" value="{$userinfo.ssn|escape}" {$autofocus} />
          {assign var=autofocus value=''}
        </td>
      </tr>
{/if}
...
{if $userinfo.tax_exempt ne "Y" or $config.Taxes.allow_user_modify_tax_number eq "Y" or $usertype eq "A" or $usertype eq "P"}
          <input type="text" id="tax_number" name="tax_number" size="32" maxlength="32" value="{$userinfo.tax_number|escape}" {$autofocus} />
          {assign var=autofocus value=''}
{else}
{$userinfo.tax_number}
{/if}
...
{/if}
{if $config.Taxes.tax_operation_scheme eq XCTaxesDefs::TAX_SCHEME_GENERAL}
...
{if $usertype eq "A" or $usertype eq "P"} 
          <input type="checkbox" id="tax_exempt" name="tax_exempt" value="Y"{if $userinfo.tax_exempt eq "Y"} checked="checked"{/if} {$autofocus} />
          {assign var=autofocus value=''}
