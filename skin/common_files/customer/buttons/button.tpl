{*
90f887473f85a3590756e3235f64ed48131fe383, v6 (xcart_4_7_6), 2016-05-18 10:43:45, button.tpl, aim
...
{elseif $style eq 'image'}
  ...
  {else}
    ...
    {strip}
      <a class="image-button{if $additional_button_class} {$additional_button_class}{/if}" {if $href}href="{$href|amp}"{else}id="{$button_id|escape|default:'image_button_id'}"{/if}
...
{elseif $is_link}
  ...
  <div class="button{if $additional_button_class} {$additional_button_class}{/if}" title="{$title|default:$button_title|strip_tags:false|escape}" onclick="{$div_link}"{if $button_id} id="{$button_id|escape}"{/if}>
    <a href="{$href}" onclick="{if $js_link}{$js_link};{else}javascript:{/if} if (event) event.cancelBubble = true;"{$reading_direction_tag}{if $target} target="{$target|strip_tags:false|escape}"{/if}>{$button_title|amp}</a>
  </div>
...
{else}
  ...
  <button class="button{if $additional_button_class} {$additional_button_class}{/if}" type="{if $type eq 'input'}submit{else}button{/if}"{if !$notitle} title="{$title|default:$button_title|strip_tags:false|escape}"{else} title="{$tips_title|escape}"{/if}{if $js_link} onclick="{$js_link}"{/if}{if $button_id} id="{$button_id|escape}"{/if}>
  {strip}
    <span class="button-right">
      <span class="button-left"{$reading_direction_tag}>{if $button_fa_icon}<i class="{$button_fa_icon}" aria-hidden="true"></i>&nbsp;{/if}{$button_title|amp}</span>
    </span>
  {/strip}
  </button>
