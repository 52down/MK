{*
a0f0edc17608d5cdc37979cca9f59b2eaed245ad, v49 (xcart_4_7_7), 2017-01-24 20:02:31, meta.tpl, aim
...
{if $usertype eq "A" or ($usertype eq "P" and $single_mode)}
constants.DIR_CUSTOMER = "{$smarty.const.DIR_CUSTOMER}";
constants.DIR_ADMIN = "{$smarty.const.DIR_ADMIN}";
constants.DIR_PROVIDER = "{$smarty.const.DIR_PROVIDER}";
constants.DIR_PARTNER = "{$smarty.const.DIR_PARTNER}";
{/if}
//]]>
</script>
...
<!--[if lt IE 9]>
  <script type="text/javascript" src="{$SkinDir}/lib/jquery-min.1x.js"></script>
<![endif]-->
<!--[if gte IE 9]><!-->
...
<script type="text/javascript" src="{$SkinDir}/lib/jquery_cookie.js"></script>
...
<script type="text/javascript" src="{if $is_https_zone}https{else}http{/if}://maps.google.com/maps/api/js?key={$config.General.google_maps_api_key}"></script>
...
{if $active_modules.XOrder_Statuses ne ""}
  {load_defer file="modules/XOrder_Statuses/common.js" type="js"}
{/if}

{if $active_modules.XAuth ne '' and $xauth_include_js eq 'Y'}
<script type="text/javascript" src="{$SkinDir}/modules/XAuth/controller.js"></script>
{/if}

{if $active_modules.Bongo_International}
  {include file="modules/Bongo_International/meta.tpl"}
{/if}
