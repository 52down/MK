{*
6f27c8266fc469c8cf1ddac70ea6b7edfdce6489, v9 (xcart_4_7_5), 2016-02-10 23:46:37, payment_methods_tab.tpl, mixon
  ...
  {capture name=dialog}
    ...
    <table cellpadding="4" cellspacing="1">
      ...
      {foreach from=$recent_payment_methods item=rpm name=rpm}
        ...
        <tr{interline class="TableSubHead" name=rpm foreach_iteration="`$smarty.foreach.rpm.iteration`" foreach_total="`$smarty.foreach.rpm.total`"}>
...
{capture name=dialog}
  ...
  <form action="payment_methods.php" method="post" name="pmform">
    ...
    <tr class="TableHead">
      <th>&nbsp;</th>
      <th width="45%">{$lng.lbl_methods}</th>
      <th width="35%" nowrap="nowrap">{$lng.lbl_special_instructions}</th>
    ...
    {section name=method loop=$payment_methods}
      ...
      <tr{if $payment_methods[method].active eq "Y"}{$active_trcolor}{else}{$inactive_trcolor}{/if}>
        ...
        <td valign="top">
          ...
          <input type="text" size="50" name="posted_data[{$payment_methods[method].paymentid}][payment_method]" value="{$payment_methods[method].payment_method|escape:"html"}" style="width: 95%;" />
          <br />
          {if $payment_methods[method].payment_script ne 'payment_giftcert.php' and not $payment_methods[method].disable_extra_charge}
            {if $payment_methods[method].processor_file ne "pp_paypal_here.php"}
            ...
            {/if}
            {if $payment_methods[method].processor_file eq "" or $payment_methods[method].processor_file eq "pp_paypal_here.php"}
              <table cellpadding="1" cellspacing="0">
                <tr>
    ...
    {if $payment_methods[method].module_name ne ""}
      <tr{if $payment_methods[method].active eq "Y"}{$active_trcolor}{else}{$inactive_trcolor}{/if}>
        <td colspan="3" valign="bottom">
          ...
          {if $payment_methods[method].note}
            {include file="main/tooltip_js.tpl" text=$payment_methods[method].note id="tip"|cat:$payment_methods[method].paymentid alt_image="icon_warning_small_red.png" type='img' sticky=true width=400}
          {/if}

          {if not $payment_methods[method].disable_checkbox}
            {if $payment_methods[method].processor neq "cc_klarna.php"}
            <a href="cc_processing.php?mode=update&amp;cc_processor={$payment_methods[method].processor}">{$lng.lbl_configure}</a>
            {elseif $payment_methods[method].processor eq "cc_klarna.php"}
              <a href="configuration.php?option=Klarna_Payments&right">{$lng.lbl_configure}</a>
            {/if}
