{*
2213c111fa2016227e48e11d2b221e41a122e680, v18 (xcart_4_7_7), 2016-07-04 13:55:47, taxes.tpl, aim
...
<select id="tax_operation_scheme" name="posted_data[tax_operation_scheme]">
    ...
    <option value="{XCTaxesDefs::TAX_SCHEME_GENERAL}" {if $config.Taxes.tax_operation_scheme eq XCTaxesDefs::TAX_SCHEME_GENERAL}selected="selected"{/if}>{$lng.opt_calculate_in_general_way}</option>
    <option value="{XCTaxesDefs::TAX_SCHEME_NO_TAXES}" {if $config.Taxes.tax_operation_scheme eq XCTaxesDefs::TAX_SCHEME_NO_TAXES}selected="selected"{/if}>{$lng.opt_do_not_add_taxes}</option>
    <option value="{XCTaxesDefs::TAX_SCHEME_NO_TAXES_FOR_VALIDATED}" {if $config.Taxes.tax_operation_scheme eq XCTaxesDefs::TAX_SCHEME_NO_TAXES_FOR_VALIDATED}selected="selected"{/if}>{$lng.opt_do_not_add_taxes_only_if_taxnumber_is_valid}</option>
    <option value="{XCTaxesDefs::TAX_SCHEME_NO_TAXES_FOR_VALIDATED_TRANSBORDER}" {if $config.Taxes.tax_operation_scheme eq XCTaxesDefs::TAX_SCHEME_NO_TAXES_FOR_VALIDATED_TRANSBORDER}selected="selected"{/if}>{$lng.opt_do_not_add_taxes_only_if_taxnumber_is_valid_transborder}</option>
</select>
    {include file="main/tooltip_js.tpl" text=$lng.txt_tax_operation_scheme_tooltip id="help_tax_operation_scheme" type='img' sticky=true wrapper_tag='div'}
    </td>
</tr>

<tr{if $config.Taxes.tax_operation_scheme eq XCTaxesDefs::TAX_SCHEME_GENERAL or $config.Taxes.tax_operation_scheme eq XCTaxesDefs::TAX_SCHEME_NO_TAXES} class="hidden"{/if}>
    <td><label for="tax_validation_exception">{$lng.opt_tax_validation_exception}:{if not $vat_checker_soap_available}{$lng.txt_soap_check_warning_for_option}{/if}</label></td>
    <td>
<select id="tax_validation_exception" name="posted_data[tax_validation_exception]">
    {* Options defined as XCTaxesDefs class constants *}
    <option value="{XCTaxesDefs::TAX_SERVICE_DOWN_CONSIDER_TN_VALID}" {if $config.Taxes.tax_validation_exception eq XCTaxesDefs::TAX_SERVICE_DOWN_CONSIDER_TN_VALID}selected="selected"{/if}>{$lng.opt_consider_tax_number_valid}</option>
    <option value="{XCTaxesDefs::TAX_SERVICE_DOWN_CONSIDER_TN_INVALID}" {if $config.Taxes.tax_validation_exception eq XCTaxesDefs::TAX_SERVICE_DOWN_CONSIDER_TN_INVALID}selected="selected"{/if}>{$lng.opt_consider_tax_number_invalid}</option>
    <option value="{XCTaxesDefs::TAX_SERVICE_DOWN_BLOCK_CHECKOUT}" {if $config.Taxes.tax_validation_exception eq XCTaxesDefs::TAX_SERVICE_DOWN_BLOCK_CHECKOUT}selected="selected"{/if}>{$lng.opt_block_checkout}</option>
</select>
    {include file="main/tooltip_js.tpl" text=$lng.txt_tax_validation_exception_tooltip id="help_tax_validation_exception" type='img' sticky=true wrapper_tag='div'}
    </td>
</tr>

<script type="text/javascript" language="JavaScript 1.2">
//<![CDATA[
  function func_tax_operation_scheme_changed() {
      var tax_operation_scheme = $(this).val();
      if (
        tax_operation_scheme === '{XCTaxesDefs::TAX_SCHEME_GENERAL}'
        || tax_operation_scheme === '{XCTaxesDefs::TAX_SCHEME_NO_TAXES}'
      ) {
        $('select#tax_validation_exception').closest('tr').addClass('hidden');
      } else {
        $('select#tax_validation_exception').closest('tr').removeClass('hidden');
      }
  }

  $(document).ready(function() {
    $('select#tax_operation_scheme').on('change', func_tax_operation_scheme_changed);
  });
//]]>
</script>
