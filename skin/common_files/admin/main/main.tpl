{*
1803c66a898d4a92da393bae7c686d548bcbd9ba, v26 (xcart_4_7_7), 2016-10-20 16:06:56, main.tpl, mixon
...
{if $current_passwords_security or $default_passwords_security or $blowfish_key_expired or $db_backup_expired or $dashboard_notifications or $not_secure_config_values or $dashboard_warnings or $dashboard_news.security_news or $dashboard_news.usual_news}
{capture name=dialog}
...
<div>
<font class="ErrorMessage">{$lng.lbl_warnings}:</font>
<ul class="admin-dashboard-notications">
  {foreach item=_warning from=$dashboard_warnings}
    <li>{$_warning}</li>
  {/foreach}
</ul>
</div>
...
<div>
<strong>{$lng.lbl_notifications}:</strong>
<ul class="admin-dashboard-notications">
  {foreach item=_notification from=$dashboard_notifications}
    <li>{$_notification}</li>
  {/foreach}
</ul>
</div>
...
$(document).ready(function () {
  ajax.core.loadBlock($('#xcart_news_items'), 'latest_xcart_news', { _request_prefix : constants.DIR_ADMIN, _request_type : 'GET'});
...
{if $active_modules.Cost_Pricing}
  {include file="modules/Cost_Pricing/admin_summary.tpl" gross_profit=$gross_profit}
{/if}
...
{section name=period loop=$item}
<tr{cycle name="col`$smarty.section.period.index`" values=', class="TableSubHead"'}>
  <td>{inc value=$smarty.section.period.index}.</td>
