{*
616cda701709b5668329a6ae4def29bb94e099b0, v7 (xcart_4_7_7), 2016-10-15 15:42:05, pages.tpl, aim
...
function prompt_about_opc_pages() {
  if ($('#pagesform_e').find('input[data-pages-used-on-opc]:checked').length > 0) {
    var txt_adm_dont_delete_aliased_pages = "{$lng.txt_adm_dont_delete_aliased_pages|wm_remove|escape:javascript}";
...
<td width="5">
  <input type="checkbox" name="to_delete[{$pages[pg].pageid}]" value="Y" {if in_array($pages[pg].filename, $pages_from_opc)}data-pages-used-on-opc="true"{/if}/>
</td>
<td>
  ...
  <hr />
    ...
    <td>
      <label for="delete_all_lng_pages">
        {$lng.lbl_static_pages_delete_all}
      </label>
    </td>
    </tr>
    {/if}
...
<tr>
  <td colspan="6" class="SubmitBox">
    <input type="button" value="{$lng.lbl_delete_selected|strip_tags:false|escape}" onclick="javascript: if (prompt_about_opc_pages() && checkMarks(this.form, new RegExp('to_delete\[[0-9]+\]', 'gi'))) submitForm(this, 'delete');" />
