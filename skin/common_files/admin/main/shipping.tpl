{*
626362abdced0086c1c116748026db88b0984243, v18 (xcart_4_7_4), 2015-10-01 14:20:13, shipping.tpl, mixon
...
<tr>
  ...
  <td>{if $config.Shipping.realtime_shipping eq "Y" and $config.Shipping.use_intershipper ne "Y" and $active_modules.UPS_OnLineTools eq "" and ($car.code eq "1800C" or $car.code eq "CPC" or $car.code eq "FDX" or $car.code eq "USPS" or $car.code eq "DHL" or $car.code eq "APOST")}<a href="shipping_options.php?carrier={$car.code}">{$lng.lbl_options} &gt;&gt;</a>{elseif $config.Shipping.realtime_shipping eq "Y" and $active_modules.UPS_OnLine_Tools and $config.Shipping.use_intershipper ne "Y" and $car.code eq "UPS"}<a href="ups.php">{$lng.lbl_ups_online_tools_configure} &gt;&gt;</a>{elseif $config.Shipping.realtime_shipping eq "Y" and $active_modules.Pitney_Bowes and $config.Shipping.use_intershipper ne "Y" and $car.code eq "PB"}<a href="shipping_options.php?carrier={$car.code}">{$lng.lbl_options} &gt;&gt;</a>{/if}</td>
</tr>
</table>
  </td>
</tr>
...
<tr{cycle values=", class='TableSubHead'"}>
  <td colspan="2"{if $s.editable_shipping eq 'Y'} class="editable"{/if}>
    {if $s.editable_shipping eq 'Y'}
      <input type="hidden" name="data[{$s.shippingid}][editable_shipping]" value="{$s.editable_shipping|escape}" />
      <input type="text" name="data[{$s.shippingid}][shipping]" size="35" value="{$s.shipping|escape}" />
    {else}
      {$s.shipping|trademark}
    {/if}
  </td>
  <td align="center"{if $s.editable_shipping_time eq 'Y'} class="editable"{/if}>
    {if $s.editable_shipping_time eq 'Y'}
      <input type="text" name="data[{$s.shippingid}][shipping_time]" size="8" value="{$s.shipping_time}" />
    {else}
      {if $s.shipping_time ne ''}{$s.shipping_time}{else}-{/if}
    {/if}
  </td>
  <td align="center">
    {if $s.destination eq "L"}{$lng.lbl_national}{else}{$lng.lbl_international}{/if}
  </td>
  <td align="center" nowrap="nowrap"{if $s.editable_weight_limit eq 'Y'} class="editable"{/if}>
    {if $s.editable_weight_limit eq 'Y'}
      <input type="text" size="8" name="data[{$s.shippingid}][weight_min]" value="{$s.weight_min|default:0|formatprice}" /> - <input type="text" size="8" name="data[{$s.shippingid}][weight_limit]" value="{$s.weight_limit|formatprice}" />
    {else}
      {if $s.weight_min ne '0.00'}{$s.weight_min|default:0|formatprice}{/if} - {if $s.weight_limit ne '0.00'}{$s.weight_limit|formatprice}{/if}
    {/if}
  </td>
...
<tr{cycle values=", class='TableSubHead'"}>
  ...
  <input type="text" name="data[{$s.shippingid}][shipping]" value="{$s.shipping|escape}" size="35" />
  </td>
...
{if $s.code eq ""}
<tr>
  <td><input type="text" name="data[{$s.shippingid}][shipping]" size="35" value="{$s.shipping|escape}" /></td>
  <td align="center"><input type="text" name="data[{$s.shippingid}][shipping_time]" size="8" value="{$s.shipping_time}" /></td>
  <td align="center"><select name="data[{$s.shippingid}][destination]">
    <option value="I"{if $s.destination eq "I"} selected="selected"{/if}>{$lng.lbl_international}</option>
    <option value="L"{if $s.destination eq "L"} selected="selected"{/if}>{$lng.lbl_national}</option>
...
<tr>
  <td><input type="text" name="add[shipping]" size="35" /></td>
