{*
2786669d22adf190c43dd388f67ad43cd959f3c7, v19 (xcart_4_7_7), 2016-10-26 10:58:37, users.tpl, mixon
...
<tr>
  <td width="5"><input type="checkbox" id="posted_data_by_username" name="posted_data[by_username]"{if not $search_prefilled or $search_prefilled.by_username} checked="checked"{/if} /></td>
  <td class="OptionLabel"><label for="posted_data_by_username">{$lng.lbl_username}</label></td>

  <td width="5"><input type="checkbox" id="posted_data_by_firstname" name="posted_data[by_firstname]"{if not $search_prefilled or $search_prefilled.by_firstname} checked="checked"{/if} /></td>
  <td class="OptionLabel"><label for="posted_data_by_firstname">{$lng.lbl_first_name}</label></td>

  <td width="5"><input type="checkbox" id="posted_data_by_lastname" name="posted_data[by_lastname]"{if not $search_prefilled or $search_prefilled.by_lastname} checked="checked"{/if} /></td>
  <td class="OptionLabel"><label for="posted_data_by_lastname">{$lng.lbl_last_name}</label></td>


  <td width="5"><input type="checkbox" id="posted_data_by_company" name="posted_data[by_company]"{if not $search_prefilled or $search_prefilled.by_company} checked="checked"{/if} /></td>
  <td class="OptionLabel"><label for="posted_data_by_company">{$lng.lbl_company}</label></td>

  <td class="OptionLabel" colspan="2">
    <select id="posted_data_by_email" name="posted_data[by_email]">
      <option value="disable_by_email"{if $search_prefilled.by_email eq "disable_by_email"} selected="selected" {/if}>{$lng.lbl_dont_search_in_email}</option>
      <option value="pattern"{if $search_prefilled.by_email eq "pattern" or ($search_prefilled and $search_prefilled.by_email eq "")} selected="selected" {/if}>{$lng.lbl_email}</option>
      <option value="start_with"{if $search_prefilled.by_email eq "start_with" or not $search_prefilled} selected="selected" {/if}>{$lng.lbl_email} {$lng.lbl_starting_with}</option>
    </select>
  </td>
</tr>
</table>
...
<tr class="TableSubHead">
  ...
  <select name="posted_data[registration_type]">
    <option value=""{if $search_prefilled.registration_type eq ""} selected="selected"{/if}>{$lng.lbl_all}</option>
    <option value="registered"{if $search_prefilled.registration_type eq "registered"} selected="selected"{/if}>{$lng.lbl_registered|lower}</option>
    <option value="anonymous"{if $search_prefilled.registration_type eq "anonymous"} selected="selected"{/if}>{$lng.lbl_anonymous}</option>
  </select>
  </td>
</tr>
...
<table cellpadding="0" cellspacing="0">
<tr>
  <td width="5"><input type="radio" id="address_type_null" name="posted_data[address_type]" value=""{if not $search_prefilled or $search_prefilled.address_type eq ""} checked="checked"{/if} onclick="javascript:managedate('address',true)" /></td>
...
<tr>
  <td colspan="2"><br />
    ...
    <script type="text/javascript" language="JavaScript 1.2">
    //<![CDATA[
      {if not $search_prefilled or $search_prefilled.address_type eq ""}
...
<form action="process_user.php" method="post" name="processuserform" id="processuserform" >
...
{foreach from=$users item=u name=users}
  ...
  <tr{interline name=users foreach_iteration="`$smarty.foreach.users.iteration`" foreach_total="`$smarty.foreach.users.total`" class=TableSubHead}>
    <td width="5"><input type="checkbox" name="user[{$u.id}]"{if $u.id eq $logged_userid} disabled="disabled" class='ui-state-disabled'{/if} /></td>
    <td>
      <a href="{if $u.id eq $logged_userid}register.php?mode=update&amp;from_search_page=1{else}user_modify.php?user={$u.id|escape:"url"}&amp;usertype={$u.usertype}{$pagestr|amp}{/if}" title="{$lng.lbl_modify_profile|escape}"><font class="ItemsList">{$uname|default:$u.login}</font></a>
      {if $uname ne $u.email}
        <br />{$u.email}
      {/if}
    </td>
    <td>
      ...
      {/if}
      {if $u.is_anonymous_customer}
        <br /><font class="SmallText"><i>({$lng.lbl_anonymous})</i></font>
      {/if}
    </td>
