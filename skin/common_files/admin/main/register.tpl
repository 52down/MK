{*
8ef06dbf1e36ce8658d6e07eb7c159ea0cb24601, v14 (xcart_4_7_6), 2016-06-10 19:00:50, register.tpl, aim
...
{else}
  ...
  {else}
    ...
    {if $smarty.get.usertype eq 'C'}
      {if $userinfo.is_anonymous_customer}
        {assign var="title" value=$lng.lbl_modify_customer_anonymous_profile}
      {else}
...
<form action="{$register_script_name}?{$php_url.query_string|escape}" method="post" name="registerform" onsubmit="javascript: return checkRegFormFields(this);" {if $config.User_Profiles.skip_js_validation_admin eq "Y"}class="skip-auto-validation"{/if}>
...
<tbody>
  ...
  {if $active_modules.XPayments_Connector}
    {include file="modules/XPayments_Connector/saved_cards_admin.tpl"}    
  {/if}
...
<tr>
  ...
  <td>
    ...
    <font class="FormButton">
      ...
      {/if}
      <input type="submit" value=" {if $userinfo.id gt 0}{if $userinfo.is_anonymous_customer}{$lng.lbl_update_anonymous_profile|escape}{else}{$lng.lbl_update|escape}{/if}{else}{$lng.lbl_register|escape}{/if} " />
    </font>
  </td>
</tr>
</tbody>
...
{if $active_modules.XPayments_Connector and $xpc_allow_add_new_card}
  {include file="modules/XPayments_Connector/saved_cards_add_new_admin.tpl"}
{/if}
