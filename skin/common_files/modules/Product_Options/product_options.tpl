{*
a963b462c9fab3af2fae978318dcfbcb319ea933, v7 (xcart_4_7_4), 2015-08-13 18:04:25, product_options.tpl, aim
...
<tr{cycle name="exceptions" values=', class="TableSubHead"'}>
  ...
  <td>{foreach $o as $v}
    {if $v@first}
      <i class="{if $v.hide_on_frontpage eq 'Y'}fa fa-eye-slash{else}icon fa fa-eye{/if}"></i>
    {/if} {*font-awesome.min.css is used*}
    <span style="white-space: nowrap; {if not $v@first}padding-left:2em{/if}">{$v.class}:&nbsp;
...
<tr>
  ...
  {/foreach}
  <tr>
    <td>{$lng.lbl_hide_exception}:</td>
    <td><input type="checkbox" name="hide_exception_on_frontpage" value="Y" checked="checked"/></td>
  </tr>
