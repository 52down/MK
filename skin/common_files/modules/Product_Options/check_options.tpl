{*
b9cf72503764b09e25f3db9f8c958fb857441c01, v12 (xcart_4_7_7), 2016-12-27 20:27:17, check_options.tpl, aim
...
{/if}
{if $v.variant_extra_fields}
{foreach from=$v.variant_extra_fields item=ef}
{if $ef.active eq "Y"}
variants[{$k}][4][{$ef.fieldid}] = '{$ef.field_value|wm_remove|escape:javascript}';
{/if}
{/foreach}
{/if}
{/foreach}
...
{if $product_options_ex2hide ne ''}
exceptions2hide = [];
{foreach from=$product_options_ex2hide item=v key=k}
exceptions2hide[{$k}] = [];
{foreach from=$v item=o}
exceptions2hide[{$k}][{$o.classid}] = {$o.optionid};
{/foreach} 
{/foreach}
{/if}
...
{if $v.active eq "Y" and $v.field_value ne ''}
product_extra_fields[{$v.fieldid}] = '{$v.field_value|wm_remove|escape:javascript}';
{/if}
{/foreach}
{/if}
...
var po_js_use_list_price_variants = '{$config.Product_Options.po_use_list_price_variants|escape:javascript}';
var dmo_need_update_block = {if $active_modules.Dont_Miss_Out}true{else}false{/if};
