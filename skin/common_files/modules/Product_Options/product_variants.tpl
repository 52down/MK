{*
3bd5d7f9a4dd899cc083e92e22881bf544d62688, v10 (xcart_4_7_7), 2016-11-20 17:36:18, product_variants.tpl, aim
...
<tr class="TableHead"> 
  ...
  {if $active_modules.POS_System}
    {$_colspan=$_colspan+1}
    <td class="DataTable">{$lng.lbl_pos_upc}</td>
  {/if}
  <td class="DataTable">{$lng.lbl_weight}</td>
  <td class="DataTable">{$lng.lbl_in_stock}</td>
  <td class="DataTable">{$lng.lbl_price}</td>
  {if $config.Product_Options.po_use_list_price_variants eq 'Y'}
    {$_colspan=$_colspan+1}
    <td class="DataTable">{$lng.lbl_list_short_price}</td>
  {/if}
  <td>{$lng.lbl_def}</td>
</tr>
{foreach from=$variants item=v key=k}
<tr{cycle name="classes" values=', class="TableSubHead"'}>
  ...
  <td class="DataTable"><input type="text" size="25" name="vs[{$k}][productcode]" value="{$v.productcode|escape}" maxlength="32"{if $v.sku_err} class="error-field"{/if} /></td>
  {if $active_modules.POS_System}
    {pos_get_html_field name="vs[$k][upc]" upc_data=$v}
  {/if}
  <td class="DataTable"><input type="text" size="4" name="vs[{$k}][weight]" value="{$v.weight|formatprice}" /></td>
  <td class="DataTable"><input type="text" size="3" name="vs[{$k}][avail]" value="{$v.avail}" /></td>
  <td nowrap="nowrap" class="DataTable"><input type="text" size="6" name="vs[{$k}][price]" value="{$v.price|formatprice}" />
    {if $v.wholesale ne ''}
  ...
  {if $config.Product_Options.po_use_list_price_variants eq 'Y'}
    <td nowrap="nowrap" class="DataTable"><input type="text" size="6" name="vs[{$k}][list_price]" value="{$v.list_price|formatprice}" /></td>
  {/if}
...
<tr id="box{$k}wp" style="display: none;">
    {if $geid ne ''}<td width="15" class="TableSubHead">&nbsp;</td>{/if}
  <td>&nbsp;</td>
  <td colspan="{$_colspan}" align="right" width="100%">
...
<tr>
  <td align="center">{$wp.quantity}</td>
