{*
5ad4e34e669e2c8fc3ca90d4a0282e1220d4a0e4, v7 (xcart_4_7_5), 2015-12-24 01:20:59, returns.tpl, mixon
...
<tr{cycle values=', class="TableSubHead"'}>
  ...
  <td valign="top"><a href="{$catalogs.admin}/user_modify.php?user={$v.all_userid}&amp;usertype=C">{$v.firstname} {$v.lastname}</a></td>
  <td valign="top">
...
{section name=amnt loop=$v.order_amount+1 start=1}
      <option value="{$smarty.section.amnt.index}"{if $v.amount eq $smarty.section.amnt.index} selected="selected"{/if}>{$smarty.section.amnt.index}</option>
{/section}
    </select>
  </td>
  <td align="center">
...
{section name=aamnt loop=$v.order_amount+1 start=0}
      <option value="{$smarty.section.aamnt.index}"{if $v.returned_amount eq $smarty.section.aamnt.index} selected="selected"{/if}>{$smarty.section.aamnt.index}</option>
{/section}
    </select>
  </td>
