{*
f33a23d411df57591a031ac0314f2f17dcc791d8, v18 (xcart_4_7_8), 2017-02-14 14:47:12, edit_products.tpl, mixon

vim: set ts=2 sw=2 sts=2 et:
*}
{capture name=js_ajax_search_by_sku_fields_aom}
  var autocomplete_field_selector = "#newproduct";
  var js_ajax_session_quick_key = "{get_ajax_session_quick_key|escape:javascript}";
{/capture}
{load_defer file="js_ajax_search_by_sku_fields_aom" direct_info=$smarty.capture.js_ajax_search_by_sku_fields_aom type="js"}
...
{function name="aom_exchange_button"}
<div class="aom-exchange">
  <button type="button" class="fa fa-exchange aom-get-price" title="{$lng.txt_aom_get_price}"
      data-aom-productid="{$product.productid}"
      data-aom-sectionid="{$smarty.section.prod_num.index}"
      data-aom-membershipid="{$cart_customer.membershipid}"
      data-aom-price-field="{$price_field}">
    <span class='ui-button-text'>{$lng.lbl_aom_get_price}</span>
  </button>
</div>
{/function}
  ...
  <td valign="top" class="aom-current aom-product-price">
    ...
    {else}
      ...
      {else}
        ...
        {if $product.is_deleted ne "Y"}
        {aom_exchange_button price_field="price"}
        {/if}
...
<tr class="TableSubHead" id="tr_aom_display_price_{$product.productid}">
  ...
  <td valign="top" class="aom-current aom-display-price">
    {if $product.is_calculated_price eq 'Y'}
      {if $orig_product.extra_data.taxed_price}
        {assign var="product_orig_display_price" value=$orig_product.extra_data.taxed_price}
      {elseif $orig_product.display_price}
      ...
      {/if}
      <input type="text" name="product_details[{$smarty.section.prod_num.index}][display_price_alt]" size="5" maxlength="15" value="{$product.taxed_price|formatprice}" {include file="main/attr_orig_data.tpl" data_orig_value=$product_orig_display_price|formatprice data_orig_keep_empty='Y'} />
      {if $product.is_deleted ne "Y"}
      {aom_exchange_button price_field="display_price_alt"}
      {/if}
      <div class="aom-catalog-price">
        {$lng.lbl_aom_catalog_price}: {if $product.is_deleted ne "Y"}{currency value=$product.catalog_price}{else}{$lng.txt_not_available}{/if}
      </div>
  ...
  <td valign="top" class="aom-original aom-display-price">
    {if $orig_product.extra_data.taxed_price}
      {currency value=$orig_product.extra_data.taxed_price}
    {else}
...
<tr>
  ...
  <td><input size="64" name="newproduct" id="newproduct" placeholder="{$lng.lbl_enter_skus|strip_tags:false|escape}" /></td>
