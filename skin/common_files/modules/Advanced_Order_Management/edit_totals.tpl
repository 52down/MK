{*
8ce2d70794c056f529ca8d6cd9d1f7f2b52371b4, v19 (xcart_4_7_6), 2016-03-14 13:54:00, edit_totals.tpl, mixon
...
{capture name=dialog}
    ...
    <form action="order.php" method="post" name="edittotals_form">
      ...
      <table cellspacing="1" cellpadding="5" width="100%">
        ...
        <tr class="TableSubHead" id="tr_payment_surcharge_other">
          <td>{$lng.lbl_other}:&nbsp;<input type="checkbox" name="total_details[use_payment_surcharge_alt]" onclick="javascript: MarkElement(this)"{if $cart_order.use_payment_surcharge_alt eq 'Y'} checked="checked"{/if} />
            ...
            <select name="total_details[payment_surcharge_type_alt]"{if $cart_order.use_payment_surcharge_alt ne 'Y'} disabled="disabled"{/if}>
              ...
              <option value="absolute"{if $cart_order.payment_surcharge_type_alt eq "absolute" or $cart_order.payment_surcharge_type_alt eq ""} selected="selected"{/if}>{$config.General.currency_symbol}</option>
            </select>
          </td>
        </tr>
        ...
        {if $config.Shipping.realtime_shipping eq "Y" and $config.Shipping.use_intershipper ne "Y" and (not $active_modules.UPS_OnLine_Tools or $show_carriers_selector ne 'Y' or $current_carrier ne 'UPS') and $dhl_ext_countries and $has_active_dhl_smethods}
            <tr{cycle values=', class="TableSubHead"'}>
