{*
f796cd4ac105eb160691c59c49e70773bd16b2d8, v18 (xcart_4_7_7), 2016-09-01 16:39:33, coupons.tpl, mixon

vim: set ts=2 sw=2 sts=2 et:
*}
{include file="page_title.tpl" title=$lng.lbl_store_coupons_title}

{capture name=js_ajax_search_by_sku_fields_coupons}
  var autocomplete_field_selector = "#productname";
  var js_ajax_session_quick_key = "{get_ajax_session_quick_key|escape:javascript}";
{/capture}
{load_defer file="js_ajax_search_by_sku_fields_coupons" direct_info=$smarty.capture.js_ajax_search_by_sku_fields_coupons type="js"}
...
<tr{cycle values=", class='TableSubHead'" advance=false}>
  ...
  <td align="center" nowrap="nowrap" {if $coupons[prod_num].expire le $smarty.now}style="background-color: #faa;"{/if}>{$coupons[prod_num].expire|date_format:$config.Appearance.datetime_format}</td>
</tr>

<tr{cycle values=", class='TableSubHead'"}>
  <td colspan="7">
{if $coupons[prod_num].productid ne 0}
{$lng.lbl_coupon_contains_product|substitute:"productid":$coupons[prod_num].productid:"path":$catalogs.customer}
...
<tr>
  ...
  <td width="100%"><input type="text" size="25" maxlength="16" name="coupon_new" value="{$coupon_data.coupon_new|escape}" autofocus="autofocus" />{if $smarty.get.error eq "coupon_already_exists"}<font class="ErrorMessage"> &lt;&lt; {$lng.lbl_coupon_already_exists}</font>{/if}</td>
</tr>
...
<tr>
  ...
  <td>&nbsp;</td>
  <td>
{include file="main/datepicker.tpl" name="expire_new" date=$coupon_data.expire_new|default:$default_coupon_new_date start_year="c-2" end_year="+5"}
  </td>
</tr>
...
<tr>
  ...
  <input size="25" name="productname" id="productname" value="{$coupon_data.productname|escape}" placeholder="{$lng.lbl_enter_sku|strip_tags:false|escape}" />
  <input type="button" onclick="javascript: popup_product('coupon_form.productid_new','coupon_form.productname');" value="{$lng.lbl_browse_|strip_tags:false|escape}" tabindex="-1" />
  </td>
</tr>
...
<tr>
  ...
  <td>{$lng.lbl_coupon_apply_category}<br />{include file="widgets/categoryselector/categoryselector.tpl" name="categoryid_new"}<br />
