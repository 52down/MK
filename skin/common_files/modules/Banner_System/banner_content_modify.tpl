{*
f796cd4ac105eb160691c59c49e70773bd16b2d8, v10 (xcart_4_7_7), 2016-09-01 16:39:33, banner_content_modify.tpl, mixon
...
//<![CDATA[
  $(function() {ldelim}
    var _storage_key_base = 'banner-system-tabs';
...
<div id="ui-tabs-container">
  ...
  <ul>
    ...
    <li>
      <a href="#bs_change_categories">{$lng.lbl_categories}</a>
    </li>
  </ul>
  ...
  <div id="upload_html">
    {include file="main/textarea.tpl" name="html_banner" cols="65" rows="15" entity_id=$bannerid entity_type='banner_system_html_banner'}
    <br />
    <input type="submit" value="{$lng.lbl_bs_add_content|escape}" />
  </div>

  <div id="bs_change_categories">
    {include file="modules/Banner_System/category_selector.tpl" id="categoryselect`$bannerid`" name="bs_categoryids[]" banner=$bs_current_banner categories=$banner_categories.$bannerid}
    <br />
...
{literal}
  $(document).ready(function() {
    var selected = $( "#ui-tabs-container" ).tabs( "option", "active" );
    if (selected == 0) {
      $("input[name=banner_type]").val("image");
    } else if (selected == 1) {
      $("input[name=banner_type]").val("html");
    } else {
      $("input[name=banner_type]").val("bs_change_categories");
    }
