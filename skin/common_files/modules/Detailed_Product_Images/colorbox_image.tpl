{*
560248120aea4394a3d11152a1e1e4789338b825, v17 (xcart_4_7_7), 2016-08-26 15:59:20, colorbox_image.tpl, mixon
...
{if $config.Detailed_Product_Images.det_image_icons_box eq 'Y'}
  <div class="dpimages-icons-box">
    {foreach from=$images item=i name=images}
      <a href="{$i.image_url|amp}" class="lightbox"{if $config.Detailed_Product_Images.det_image_icons_limit gt 0 and $config.Detailed_Product_Images.det_image_icons_limit le $smarty.foreach.images.index} style="display:none;"{/if} rel="dpimages" title="{$i.alt|truncate:"35":"..."|escape}"><img src="{$i.icon_url|amp}" alt="{$i.alt|escape}" title="{$i.alt|escape}" width="{$i.icon_image_x}" height="{$i.icon_image_y}" /></a>
    {/foreach}
