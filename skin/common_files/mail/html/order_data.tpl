{*
0559ca8bda5978a4247cd80d0c9c0635f06a7290, v18 (xcart_4_7_8), 2017-06-01 10:08:00, order_data.tpl, Ildar
...
<table {if $is_nomail eq 'Y'}cellspacing="1" class="invoice-products"{else}cellspacing="0" cellpadding="3" width="100%" border="1"{/if} summary="{$lng.lbl_products|escape}">
  ...
  <tr>
    ...
    {/if}
    <th {if $is_nomail eq 'Y'}class="invoice-price-column"{else}nowrap="nowrap" width="100" bgcolor="#cccccc"{/if}>{$lng.lbl_item_price}</th>
  ...
  {foreach from=$products item=product}
    ...
    <tr>
      <td {if $is_nomail eq 'Y'}class="invoice-price-column"{else}align="center"{/if}>{$product.productcode}{if $active_modules.ShippingEasy ne '' and $order.status eq 'S' and $product.shipped_se eq 'Y'} (<strong style="color: red;">{$lng.lbl_shippingeasy_shipped}<strong>){/if}</td>
...
<table cellspacing="0" {if $is_nomail eq 'Y'}class="invoice-totals"{else}cellpadding="0" width="100%" border="0"{/if} id="html_order_info_total" summary="{$lng.lbl_total|escape}">
  ...
  {if $config.Shipping.enable_shipping eq 'Y' or $order.display_shipping_cost gt 0}
    <tr>
    ...
    {if $active_modules.Pitney_Bowes}
      {include file="modules/Pitney_Bowes/shipping_details.tpl"}
    {/if}
  {/if}
  ...
  {if $order.applied_taxes and $order.extra.tax_info.display_taxed_order_totals ne "Y"}
    {foreach key=tax_name item=tax from=$order.applied_taxes}
      <tr>
        {include file="main/display_tax_rate.tpl" value=$tax.rate_value assign="rate_value"}
        <td {if $is_nomail eq 'Y'}class="invoice-total-name"{else}style="padding-right: 3px; height: 20px; width: 100%; text-align: right;"{/if}><strong>{$tax.tax_display_name}{if $tax.rate_type eq "%"} {$rate_value}%{/if}:</strong></td>
  ...
  {if $_userinfo.tax_exempt ne "Y"}
    ...
    {if $order.applied_taxes and $order.extra.tax_info.display_taxed_order_totals eq "Y"}
      {foreach key=tax_name item=tax from=$order.applied_taxes}
        <tr>
          {include file="main/display_tax_rate.tpl" value=$tax.rate_value assign="rate_value"}
          <td {if $is_nomail eq 'Y'}class="invoice-total-name"{else}style="padding-right: 3px; height: 20px; width: 100%; text-align: right;"{/if}><strong>{$lng.lbl_including_tax|substitute:"tax":$tax.tax_display_name}{if $tax.rate_type eq "%"} {$rate_value}%{/if}:</strong></td>
          <td {if $is_nomail eq 'Y'}class="invoice-total-value"{else}style="white-space: nowrap; padding-right: 5px; height: 20px; text-align: right;"{/if}>{currency value=$tax.tax_cost}</td>
        </tr>
      {/foreach}
    {/if}
  ...
  <table cellspacing="1" {if $is_nomail eq 'Y'}class="invoice-giftcerts"{else}cellpadding="0" width="100%" border="0"{/if} summary="{$lng.lbl_gift_certificates|escape}">
    ...
    {foreach from=$order.applied_giftcerts item=gc}
      <tr>
        ...
        <td {if $is_nomail eq 'Y'}class="invoice-giftcert-cost"{else}style="text-align: right; white-space: nowrap; padding-right: 5px;"{/if}>{currency value=$gc.giftcert_cost}</td>
      </tr>
    {/foreach}
...
{if $order.extra.bongo_checkout ne "" and $active_modules.Bongo_International}
  {include file="modules/Bongo_International/mail/html/order_data.tpl"}
{/if}
...
{if $active_modules.Pitney_Bowes}
  {include file="modules/Pitney_Bowes/order_tracking.tpl" is_nomail=$is_nomail}
{/if}
