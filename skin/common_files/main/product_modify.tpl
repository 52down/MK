{*
41562c5cfeb62cd856c8982b2d194c8a49e4f4c3, v18 (xcart_4_7_7), 2016-12-26 19:10:26, product_modify.tpl, aim
...
<span class='product-title'>
  {$product.product|truncate:72:"...":false}
</span>
<br />
{if $active_modules.New_Arrivals}
  {include file="modules/New_Arrivals/new_arrivals_product_modify_date.tpl" product=$product}
{/if}
{/if}
...
<tr{cycle name="ge" values=', class="TableSubHead"'}>
  <td>{if $productid eq $v.productid}<b>{else}<a href="product_modify.php?productid={$v.productid}{if $section ne 'main'}&amp;section={$section}{/if}&amp;geid={$geid}{if $navigation_page}&amp;page={$navigation_page}{/if}">{/if}
...
{if $active_modules.Product_Options and $active_modules.Extra_Fields and $product.is_variants eq "Y" and $section eq "variants_extra_fields" and not $is_pconf}
  <a name="section_variants_extra_fields"></a>
  {include file="modules/Product_Options/product_variants_extra_fields_modify.tpl"}
  <br />
{/if}
...
{if $active_modules.Advanced_Customer_Reviews and $section eq "acr_reviews"}
{include file="modules/Advanced_Customer_Reviews/acr_product_modify.tpl"}
{/if}
...
{if $active_modules.Refine_Filters and $section eq "custom_class"}
<a name="section_custom_class"></a>
{include file="modules/Refine_Filters/rf_product_class.tpl"}
<br />
{/if}

{if $active_modules.Pitney_Bowes and $section eq "product_restrictions"}
{include file="modules/Pitney_Bowes/product_restrictions.tpl"}
{/if}

{if $active_modules.POS_System and $section eq "barcode"}
{include file="modules/POS_System/product_section_barcode.tpl"}
{/if}

{if $active_modules.Amazon_Feeds and $section eq "feeds_details"}
{include file="modules/Amazon_Feeds/product_feeds_details.tpl"}
{/if}

{if $active_modules.Bongo_International and $section eq "bongo"}
  <a name="section_bongo"></a>
  {include file="modules/Bongo_International/product_modify.tpl"}
  <br />
{/if}
