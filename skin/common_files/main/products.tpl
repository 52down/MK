{*
52282f5437b11f12975182697726800d7124d099, v6 (xcart_4_7_3), 2015-06-04 17:48:33, products.tpl, mixon
...
{/if}
  {if $active_modules.New_Arrivals}
    {include file="modules/New_Arrivals/new_arrivals_product_list_date.tpl" is_header="Y"}
  {/if}
...
<tr{cycle values=', class="TableSubHead"'}>
  ...
  <td width="100%">{if $products[prod].main eq "Y" or $main ne "category_products"}<b>{/if}<a href="product_modify.php?productid={$products[prod].productid}{if $navpage}&amp;page={$navpage}{/if}">{$products[prod].product}</a>{if $products[prod].main eq "Y" or $main ne "category_products"}</b>{/if}{if $active_modules.Alibaba_Wholesale and $products[prod].alibaba_wholesale_id}<a class="aw-product-sign" href="alibaba_wholesale_catalog.php?action=redirect&aw_product_id={$products[prod].alibaba_wholesale_id|urlencode}" target="_blank"></a> {* XCAlibabaWholesaleDefs->TYPE_REDIRECT *} {/if}</td>
{if $main eq "category_products"}
  <td><input type="text" size="9" maxlength="10" name="posted_data[{$products[prod].productid}][orderby]" value="{$products[prod].orderby}" /></td>
{/if}
  {if $active_modules.New_Arrivals}
    {include file="modules/New_Arrivals/new_arrivals_product_list_date.tpl" product=$products[prod]}
  {/if}
  <td align="center">
