{*
ba0de6b489855a9995f4a98fa9fe5a7bd1b57fb4, v14 (xcart_4_7_4), 2015-08-17 14:49:58, popup_users.tpl, aim
...
<table cellpadding="0" cellspacing="0" width="{$width|default:"100%"}" align="center" style="padding: 5px;">
<tr>
...
searchform_def[3] = ['posted_data[by_email]', 'pattern'];
...
searchform_def[10] = ['posted_data[registration_type]', ''];
//]]>
</script>
...
<input type="hidden" name="single_selection" value="{$single_selection|escape}" />
...
<tr>
  ...
  <td class="OptionLabel" colspan="2">
    <select id="posted_data_by_email" name="posted_data[by_email]">
      ...
      <option value="pattern"{if $search_prefilled.by_email eq "pattern" or $search_prefilled.by_email eq ""} selected="selected" {/if}>{$lng.lbl_email}</option>
      <option value="start_with"{if $search_prefilled.by_email eq "start_with"} selected="selected" {/if}>{$lng.lbl_email} {$lng.lbl_starting_with}</option>
    </select>
  </td>
...
<tr{cycle values=', class="TableSubHead"'}>
  <td width="5">
    {if $single_selection ne 'Y'}
      <input type="checkbox" name="user[{$users[cat_num].id}]"{if $users[cat_num].id eq $logged_userid} disabled="disabled" class='ui-state-disabled'{/if} />
    {else}
      <input type="radio" name="user" value="{$users[cat_num].id}"{if $smarty.section.cat_num.first} checked="checked"{/if} />
    {/if}
  </td>
...
{if $users[cat_num].is_anonymous_customer}
...
<tr>
  ...
  {else}
    <input type="button" value="{$lng.lbl_submit|strip_tags:false|escape}" onclick="javascript: submitForm(this, 'save');" />
  {/if}
  </td>
</tr>
