{*
0559ca8bda5978a4247cd80d0c9c0635f06a7290, v28 (xcart_4_7_8), 2017-06-01 10:08:00, orders_list.tpl, Ildar
...
<form action="process_order.php" method="post" name="processorderform" id="processorderform" >
...
{if $active_modules.AfterShip ne ''}
  <td nowrap="nowrap">{if $search_prefilled.sort_field eq "tracking"}{include file="buttons/sort_pointer.tpl" dir=$search_prefilled.sort_direction}&nbsp;{/if}<a href="orders.php?mode=search&amp;sort=tracking{if $search_prefilled.sort_field eq "tracking"}&amp;sort_direction={if $search_prefilled.sort_direction eq 1}0{else}1{/if}{/if}">{$lng.lbl_aftership_tracking_N}</a></td>
{/if}
...
{$total=$total+$orders[oid].total}
{if $active_modules.Cost_Pricing}
  {$gross_costs_total=$gross_costs_total+$orders[oid].costs_total}
{/if}
{if $orders[oid].status eq "P" or $orders[oid].status eq "C"}
  {$total_paid=$total_paid+$orders[oid].total}
{/if}

<tr{cycle values=", class='TableSubHead'"}>
  <td width="5"><input type="checkbox" name="orderids[{$orders[oid].orderid}]" /></td>
  <td><a href="order.php?orderid={$orders[oid].orderid}" title="{$orders[oid].item_productcodes|strip_tags:false|escape}">#{$orders[oid].orderid}</a></td>
  <td nowrap="nowrap">
{if $is_admin_user}
<input type="hidden" name="order_status_old[{$orders[oid].orderid}]" value="{$orders[oid].status}" />
{if $active_modules.XPayments_Connector && $orders[oid].xpc_antifraud_result}
  {include file="modules/XPayments_Connector/antifraud_status.tpl" status=$orders[oid].xpc_antifraud_result orderid=$orders[oid].orderid}
{/if}
{if $orders[oid].status_blocked}
...
{/if}
{if $active_modules.POS_System && $orders[oid].is_pos_order}<img title="{$lng.lbl_pos_order|strip_tags:false|escape}" alt="{$lng.lbl_pos_order}" src="{$SkinDir}/modules/POS_System/images/pos_order.svg">{/if}
...
{if $active_modules.AfterShip ne ''}
  <td nowrap="nowrap" align="{if $orders[oid].tracking ne ""}right{else}center{/if}">
    {include file="modules/AfterShip/admin/aftership_tracking_admin.tpl" data_width="0" data_size="small" tracking_number=$orders[oid].tracking order_id=$orders[oid].orderid}
  </td>
{/if}
  <td>
    ...
    {else}
      ...
      {/if}
    {/if}
    {include file="gmap.tpl" address=$orders[oid].gmap.address description=$orders[oid].gmap.description parent_tpl='orders_list'}
  </td>
{if $usertype eq "A" and $single_mode eq ""}
...
<input type="button" value="{$lng.lbl_shippingeasy_export_to|strip_tags:false|escape}" onclick="javascript: if (checkMarks(this.form, new RegExp('orderids\[[0-9]+\]', 'gi'))) {ldelim} document.processorderform.action='shippingeasy_export.php'; submitForm(this, ''); {rdelim}" />
&nbsp;&nbsp;&nbsp;&nbsp;
{/if}
