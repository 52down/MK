{*
2786669d22adf190c43dd388f67ad43cd959f3c7, v17 (xcart_4_7_7), 2016-10-26 10:58:37, orders.tpl, mixon
...
<tr>
  <td height="10" width="20%" class="FormButton" nowrap="nowrap" valign="top" style="padding-top: 14px;">{$lng.lbl_date_period}:</td>
  <td>

<table cellpadding="2" cellspacing="2">

<tr>
  <td colspan="2">{include file="widgets/daterangepicker/daterangepicker.tpl" value=$search_prefilled.date_range}</td>
</tr>
...
{if $active_modules.POS_System}
  {include file="modules/POS_System/orders.tpl"}
{/if}
...
<tr>
  <td class="FormButton" nowrap="nowrap">{$lng.lbl_email}
    <input type="radio" id="email_search_type_pattern" name="posted_data[email_search_type]" value="pattern"{if $search_prefilled.email_search_type eq "pattern"} checked="checked"{/if} /><label for="email_search_type_pattern">{$lng.lbl_pattern}</label>
    <input type="radio" id="email_search_type_start_with" name="posted_data[email_search_type]" value=""{if $search_prefilled.email_search_type eq "start_with" or $search_prefilled.email_search_type eq ""} checked="checked"{/if} /><label for="email_search_type_start_with">{$lng.lbl_starting_with}</label>
  </td>
...
<tr>
  <td class="FormButton" nowrap="nowrap">{$lng.lbl_xps_subscriptions}:</td>
  <td>
    <script type="text/javascript">searchform_def.push(['posted_data[subscriptions_orders]', 'A']);</script>
    <select name="posted_data[subscriptions_orders]" style="width:70%">
      <option value="A"{if $search_prefilled.subscriptions_orders eq "A"} selected="selected"{/if}>{$lng.lbl_xps_all_orders}</option>
      <option value="S"{if $search_prefilled.subscriptions_orders eq "S"} selected="selected"{/if}>{$lng.lbl_xps_subscription_orders}</option>
      <option value="N"{if $search_prefilled.subscriptions_orders eq "N"} selected="selected"{/if}>{$lng.lbl_xps_none_subscription_orders}</option>
    </select>
  </td>
</tr>
{/if}
