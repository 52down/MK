{*
1570ad059fc3dd81e1e3723f1770f61d5e2c6d59, v44 (xcart_4_7_8), 2017-06-01 15:37:47, history_order.tpl, aim
...
{if $active_modules.Bongo_International}
  {include file="modules/Bongo_International/order_info_header.tpl"}
{/if}
...
{if $active_modules.Klarna_Payments}
  {include file="modules/Klarna_Payments/order_klarna_options_button.tpl"}
{/if}
</tr>
</table>
  </td>
...
{if $active_modules.Pitney_Bowes ne ""}
  {include file="modules/Pitney_Bowes/order_parcels_button.tpl"}
{/if}
{if $active_modules.POS_System ne ""}
  {include file="modules/POS_System/receipt/admin_print_receipt_button.tpl"}
{/if}
...
<td class="ButtonsRowRight">
  {include file="buttons/button.tpl" button_title=$lng.lbl_print_packing_slip target="_blank" href="order.php?orderid=`$order.orderid`&mode=packing_slip" substyle="link"}
</td>
{if $active_modules.ShippingEasy ne '' and ($usertype eq 'A' or $usertype eq 'P')}
  {getvar var='shippingeasy' func='func_shippingeasy_tpl_get_se_export_status' orderid=$order.orderid}
  {if $shippingeasy ne 'Y'}
  <td class="ButtonsRowRight">
    {include file="buttons/button.tpl" button_title=$lng.lbl_shippingeasy_export_to href="shippingeasy_export.php?orderid=`$order.orderid`" substyle="link"}
  </td>
  {/if}
{/if}
<td class="ButtonsRowRight">
...
{if $active_modules.Amazon_Payments_Advanced}
  {include file="modules/Amazon_Payments_Advanced/order_controls.tpl"}
{/if}
...
<strong>{$lng.lbl_xpc_not_charged}</strong>
<br/>

<form action="order.php?mode=xpc_charge" method="post">

  <input type="hidden" name="orderid" value="{$order.orderid}">
  {include file="modules/XPayments_Connector/card_list_admin_recharge.tpl"}
  
  <br/><br/>
  {$lng.lbl_amount}: <input type="text" name="recharge_total" value="{$order.total}">
...
<form action="order.php" method="post" id="main_order_form">
...
{elseif $order.capture_enable}
  ...
  {if not $order.capture_noninit or $order.capture_limit}
    {currency value=$order.capture_full_total assign=capture_full_total plain_text_message=Y}
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    {if $order.capture_limit}
        <input type="button" value="{$lng.lbl_capture_n|substitute:cost:$capture_full_total}"{if $order.paypal.fmf} disabled="disabled"{/if}onclick="javascript: self.location = 'process_order.php?orderid={$order.orderid}&amp;mode=capture';" id="btn_capture_n" />
    {else}
        <input type="button" value="{$lng.lbl_capture_n|substitute:cost:$init_total}"{if $order.paypal.fmf} disabled="disabled"{/if}onclick="javascript: self.location = 'process_order.php?orderid={$order.orderid}&amp;mode=capture';" id="btn_capture_n" />
    {/if}
  ...
  {if $order.capture_noninit}
    ...
    {if not $order.capture_limit}
      ...
      {if $order.void_enable}
        &nbsp;&nbsp;&nbsp;&nbsp;
        {if $active_modules.Klarna_Payments and $order.extra.reservation_id neq ''}
          <input type="button" value="{$lng.lbl_klarna_cancel_payment|escape}" onclick="javascript: self.location = 'process_order.php?orderid={$order.orderid}&amp;mode=decline';" />
        {else}
          <input type="button" value="{$lng.lbl_decline}" onclick="javascript: self.location = 'process_order.php?orderid={$order.orderid}&amp;mode=void';"/>
        {/if}
      {/if}
...
{/if}
  ...
  {if $order.status eq "F" && $order.extra.paypal_fraud_info eq 'Y'}
    <br /><br />
    <strong>{$lng.lbl_note}:</strong> {$lng.lbl_paypal_fraud_info}<br />
    <br />
  {/if}
...
{if $active_modules.XPayments_Connector}
  {include file="modules/XPayments_Connector/history_order.tpl"}
{/if}
...
{if $usertype eq "A" or $usertype eq "P"}
  {if $active_modules.XPayments_Connector && $order.extra.xpc_fraud_check_data}
    <br /><br /><br />
    {include file="modules/XPayments_Connector/fraud_check_data.tpl"}
  {/if}
{/if}
...
{if ($usertype eq 'A' or ($usertype eq 'P' and $active_modules.Simple_Mode)) and $active_modules.Egoods and $order.is_egood ne ''}
<br />
...
{if $usertype eq "A" and $order.charge_info and ($order.charge_info.refunded_total gt 0 or $order.charge_info.refund_avail gt 0)}

  <br />
  {include file="main/subheader.tpl" title=$lng.lbl_refund}

  <form action="process_order.php" method="post" name="refundform" id="order_refundform">
      ...
      {if $order.charge_info.refund_avail gt 0}
        {if $order.charge_info.refund_mode eq 'P'}
          <tr>
            ...
            <td><input type="text" name="refund_amount" id="refund_amount" value="{$order.charge_info.refund_avail}" /></td>
          </tr>
        {/if}
        <tr>
          <td>&nbsp;</td>
...
{if $usertype eq 'A' or $usertype eq 'P'}
  {if $active_modules.Pitney_Bowes ne '' and $order.extra.Pitney_Bowes->suppress_leg_tracking eq 'Y'}
  {else}
    {include file="main/order_tracking_numbers.tpl" orderid=$order.orderid}
  {/if}
{/if}

{if $active_modules.AfterShip ne '' and $order.tracking ne ''}
  {include file="modules/AfterShip/aftership_tracking.tpl" data_width="0" data_size="small" tracking_number=$order.tracking}
{/if}
