{*
855e9e52d63ca79265fc2f834fcea28f59ee3c0e, v16 (xcart_4_7_7), 2016-11-14 14:53:23, popup_product.tpl, aim
...
{capture name=popup_get_params}&amp;cat={$smarty.get.cat|escape:"html"}&amp;field_product={$smarty.get.field_product|stripslashes}&amp;field_productid={$smarty.get.field_productid|stripslashes}&amp;only_regular={$smarty.get.only_regular|stripslashes}{/capture}
...
{section name=book_idx loop=$bookmarks}
  <li>
  <a href="javascript:setProduct('{$bookmarks[book_idx].productid}','{$bookmarks[book_idx].product|replace:"'":"\'"|replace:'"':"\'\'"}')">{$bookmarks[book_idx].product|truncate:72:"...":true}</a>
  &nbsp;&nbsp;
  <a href="popup_product.php?mode=delete_bookmark&amp;productid={$bookmarks[book_idx].productid}{$smarty.capture.popup_get_params}"><b>[{$lng.lbl_delete}]</b></a>
  </li>
{/section}
</ul>
{else}
...
<input type="hidden" name="top_cat" value="{$smarty.get.top_cat|escape:"html"}" />
<input type="hidden" name="field_product" value="{$smarty.get.field_product|escape:"html"|stripslashes}" />
<input type="hidden" name="field_productid" value="{$smarty.get.field_productid|escape:"html"|stripslashes}" />
<input type="hidden" name="only_regular" value="{$smarty.get.only_regular|escape:"html"|stripslashes}" />
...
{if $popup_product_1st_column eq 'product_name'}<i class="fa fa-circle"></i>&nbsp;{/if}<a href="popup_product.php?mode=set_popup_first_column&amp;_popup_first_column=product_name{$smarty.capture.popup_get_params}"><b>{$lng.lbl_product_name}</b></a>&nbsp;/&nbsp;
{if $popup_product_1st_column eq 'product_code'}<i class="fa fa-circle"></i>&nbsp;{/if}<a href="popup_product.php?mode=set_popup_first_column&amp;_popup_first_column=product_code{$smarty.capture.popup_get_params}"><b>{$lng.lbl_sku}</b></a><br />
...
{section name=prod_idx loop=$products}
    <option value="{$products[prod_idx].productid}" title="{$products[prod_idx].product|cat:' ('|cat:$products[prod_idx].productcode|strip_tags:false|escape})" {if $forsale_colors[$products[prod_idx].forsale]}style="color:{$forsale_colors[$products[prod_idx].forsale]}"{/if}>{if $popup_product_1st_column eq 'product_code'}{$products[prod_idx].productcode}&nbsp;({$products[prod_idx].product}){else}{$products[prod_idx].product} ({$products[prod_idx].productcode}){/if}</option>
{/section}
</select><br /><br />
<center>
