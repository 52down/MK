{*
0559ca8bda5978a4247cd80d0c9c0635f06a7290, v27 (xcart_4_7_8), 2017-06-01 10:08:00, order_info.tpl, Ildar
...
{*if order placed by fCommerce_Go - add img icon to product name*}
{if $products[prod_num].extra_data.added_in_facebook}
  {$products[prod_num].product="`$products[prod_num].product`
    <img src='$current_location/fb_icon.gif' alt='F' title='`$lng.lbl_fb_added_in_facebook`' style='vertical-align: top; margin-left: 5px;' />"}
{/if}
...
<tr> 
  ...
  <td valign="top">{$products[prod_num].productcode|default:"-"}{if $active_modules.ShippingEasy ne '' and ($order.status eq 'S' and $products[prod_num].shipped_se eq 'Y')} (<strong style="color: red;">{$lng.lbl_shippingeasy_shipped}<strong>){/if}</td>
</tr>
...
{if
  $customer.tax_exempt ne "Y"
  and $order.extra.tax_info.display_cart_products_tax_rates eq "Y"
  and ($products[prod_num].product_type ne "C" or $products[prod_num].display_price gt 0)
}
  {capture name=product_taxes}
      {foreach from=$products[prod_num].extra_data.taxes item=tax}
        ...
        {if $tax.tax_value gt 0}
            ...
            {/if} ({currency value=$tax.tax_value_precise})<br />
        {/if}
        </small>
      {/foreach}
  {/capture}

  {if $smarty.capture.product_taxes|strip:''}
    <tr>
      <td valign="top" style="padding-left: 15px;">{$lng.lbl_including}</td>
      <td style="white-space: nowrap;">
        {$smarty.capture.product_taxes}
      </td>
    </tr> 
  {/if}
{/if}
...
{if $products[prod_num].extra_data.subscription and $subscriptions_info}
{include file="modules/XPayments_Subscriptions/order_product_info.tpl" product=$products[prod_num]}
{/if}
{/if}
...
<tr> 
  ...
  {strip}
    ...
    {if $active_modules.XPayments_Connector and $order.extra.xpc_saved_card_num ne ''}
      {include file="modules/XPayments_Connector/order_info.tpl"}
    {/if}
...
{if $active_modules.Pitney_Bowes}
  {include file="modules/Pitney_Bowes/order_info.tpl"}
{/if}
...
{if $active_modules.Bongo_International}
  {include file="modules/Bongo_International/order_info.tpl"}
{/if}
...
<tr valign="top">
  ...
  <td class="gmap-order-info">
    <br />
    {include file="gmap.tpl" address=$customer.b_gmap.address description=$customer.b_gmap.description show_text="Y"}
  </td>
</tr>
...
<tr valign="top">
  ...
  <td class="gmap-order-info">
    <br />
    {include file="gmap.tpl" address=$customer.s_gmap.address description=$customer.s_gmap.description show_text="Y"}
  </td>
</tr>
...
<tr valign="top">
  <td colspan="2">
      <br />
      <i>{$lng.lbl_additional_information}:</i>
  </td>
</tr>
...
<p>
  <b>{$lng.lbl_note}:</b> {$lng.txt_different_order_language_note|substitute:"language":$customer.language}
</p>
{/if}
