{*
141c878b03b2b0cdd5d2b31c952f4e1031962148, v33 (xcart_4_7_8), 2017-05-18 15:44:49, product_details.tpl, aim
...
{if $active_modules.Alibaba_Wholesale}
  {include file="modules/Alibaba_Wholesale/product_details_admin.tpl"}
{/if}
...
<tr>
  ...
  <td class="ProductDetails">
    ...
    {if $top_message.fillerror ne "" and $product.categoryid eq ""}<span class="Star">&lt;&lt;</span>{/if}
  </td>
</tr>
...
<tr>
  ...
  <td class="ProductDetails">
    {include file="widgets/categoryselector/categoryselector.tpl" name="categoryids[]" class="InputWidth" selected_ids=$product.add_categoryids extra=' multiple="multiple" size="8"'}
  </td>
</tr>
...
<tr>
  ...
  {assign var=customer_product_url value=$product.internal_url|replace:https:http}
  <td class="ProductDetails"><a href="{$customer_product_url}">{$customer_product_url}</a></td>
</tr>
{/if}
...
{if $active_modules.POS_System ne ""}
  {include file="modules/POS_System/product_modify_upc.tpl"}
{/if}
...
{include file="main/textarea.tpl" name="descr" cols=45 rows=8 data=$product.descr width="100%" btn_rows=4 entity_id=$product.productid entity_type='product_descr'}
...
<div class="description-data">
  {include file="main/textarea.tpl" name="fulldescr" cols=45 rows=12 class="InputWidth" data=$product.fulldescr width="100%" btn_rows=4 entity_id=$product.productid entity_type='product_fulldescr'}
...
<tr>
  ...
  <td class="ProductDetails"><textarea name="title_tag" maxlength="255" cols="45" rows="6" class="InputWidth">{$product.title_tag}</textarea></td>
</tr>
...
<tr>
  ...
  <td class="ProductDetails"><textarea name="meta_keywords" maxlength="255" cols="45" rows="6" class="InputWidth">{$product.meta_keywords}</textarea></td>
</tr>
...
<tr>
  ...
  <td class="ProductDetails"><textarea name="meta_description" maxlength="255" cols="45" rows="6" class="InputWidth">{$product.meta_description}</textarea></td>
</tr>
...
<tr>
  ...
  <td class="ProductDetails">
  {if $product.is_variants eq 'Y' and $config.Product_Options.po_use_list_price_variants eq 'Y'}
    <b>{$lng.lbl_note}:</b> {$lng.txt_pvariant_edit_note|substitute:"href":$variant_href}
  {else}
...
{if $active_modules.Cost_Pricing ne ""}
  {include file="modules/Cost_Pricing/product_modify_cost.tpl"}
{/if}

{if $active_modules.XPayments_Subscriptions}
{include file="modules/XPayments_Subscriptions/product_details.tpl"}
{/if}
...
{if $product.is_variants ne 'Y'}{*net_weight is not supported by variants now*}
<tr>
  {if $geid ne ''}<td width="15" class="TableSubHead">{if $product.is_variants eq 'Y'}&nbsp;{else}<input type="checkbox" value="Y" name="fields[net_weight]" />{/if}</td>{/if}
  <td class="FormButton" nowrap="nowrap">{$lng.lbl_net_weight} ({$config.General.weight_symbol}):</td>
  <td class="ProductDetails">
{if $product.is_variants eq 'Y'}
<b>{$lng.lbl_note}:</b> {$lng.txt_pvariant_edit_note|substitute:"href":$variant_href}
{else}
  <input type="text" name="net_weight" size="18" value="{$product.net_weight|formatprice|default:$zero}" />
{/if}
  </td>
</tr>
{/if}
...
{if $active_modules.AvaTax ne ""}
  {if $geid ne ''}<td width="15" class="TableSubHead"><input type="checkbox" value="Y" name="fields[avatax_tax_code]" /></td>{/if}
  <td class="FormButton" nowrap="nowrap">{$lng.lbl_avatax_tax_code}:</td>
  <td class="ProductDetails"><input type="text" name="avatax_tax_code" size="18" value="{$product.avatax_tax_code}" /></td>
{/if}
...
{if $active_modules.On_Sale}
  {include file="modules/On_Sale/on_sale_product_modify_checkbox.tpl" product=$product}
{/if}
...
{if $active_modules.Facebook_Ecommerce}
  {include file="modules/Facebook_Ecommerce/product_modify.tpl" product=$product}
{/if}
      ...
      <tr>
        ...
        <td width="100%" align="right">
          {if $product.productid gt 0}
            <input type="button" value="{$lng.lbl_preview|strip_tags:false|escape}" onclick="javascript: window.open('{$catalogs.customer}/product.php?productid={$product.productid}&amp;is_admin_preview=Y', '_blank');" /> &nbsp;&nbsp;&nbsp;
