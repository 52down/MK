...
/**
 * Functions for "iDEAL: Rabobank Professional payment gateway" payment module
 *
 * @category   X-Cart
 * @package    X-Cart
 * @subpackage Lib
 ...
 * @version    cf9e608d41c40f761c6416f642a1d0094a6af214, v11 (xcart_4_7_7), 2017-01-24 09:29:34, func.cc_ideal_rb_prof.php, aim
...
class IdealProRequest /* v.3.3.1 */ {
     ...
     * Constructor
     * 
     * @global type $xcart_dir
     * @global type $var_dirs
     */
    public function __construct()
    {
        global $xcart_dir, $var_dirs;

        $ideal_params = func_get_pm_params('cc_ideal_rb_prof.php');

        $this->setSecurePath($xcart_dir . '/payment/certs/');
        $this->setCachePath($var_dirs['cache'] . '/');

        $this->setMerchant($ideal_params['param01'], $ideal_params['param02']);
        $this->setPrivateKey($ideal_params['param06'], $ideal_params['param05'], $ideal_params['param04']);

        $this->sPublicCertificateFile = $ideal_params['param03'];

        $this->setMode($ideal_params['testmode']);
    }
    ...
    {
        x_log_add('ideal-pro', $sMessage . " in $sFile:$nLine"); exit();
    }
    ...
    protected function postToHost($url, $data)
    {
        ...
        list($headers, $return) = func_https_request('POST', $url, array($data), '', '', 'text/xml');

        $result = '';

        if (substr($headers, 9, 6) !== '200 OK') {
...
class IdealProTransactionRequest extends IdealProRequest {
    ...
    public function __construct()
    {
        ...
        $returnURL = $current_location . '/payment/cc_ideal_rb_prof.php';
