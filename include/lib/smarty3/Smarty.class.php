...
class SmartyException extends Exception
{
    ...
    {
		//default print_r($this->getTrace(), true) causes memory overflow
		$trace = $this->getTrace();
		if (is_array($trace)) {
			...
			foreach ($trace as $key=>$line) {
				...
				foreach ($line['args'] as $key_arg=>$arg) {
					if (is_object($arg)) {
						$trace[$key]['args'][$key_arg] = get_class($arg) . ' object is here';
					}
				}
			}
			x_load('logging');
			x_log_add('smarty_trace', $trace);
		}

        return ' --> Smarty: ' . (self::$escape ? htmlentities($this->message) : $this->message) . "\n\tBacktrace:" . ' see in var/log/x-errors_smarty_trace-<date>.php file' . ' <-- ';
    }
}
