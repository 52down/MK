 ...
 * @version    f87d18fe5743a8d67e78ec4755b3dc07e144b3a0, v326 (xcart_4_7_8), 2017-06-06 19:56:03, func.user.php, aim
...
class XCUserSql {
    public static function getSqlAnonymousCond() { // {{{
        global $sql_tbl;
        return " $sql_tbl[customers].is_anonymous_customer=1 ";
    } //}}}

    public static function getSqlRegisteredCond() { // {{{
        global $sql_tbl;
        return " $sql_tbl[customers].is_anonymous_customer=0 ";
    } //}}}

    public static function getSqlCondByAnonymousFlag($anonymous_flag = 'anonymous') { // {{{
        if ($anonymous_flag == 'anonymous')
            return self::getSqlAnonymousCond();
        else
            return self::getSqlRegisteredCond();
    } //}}}

    public static function isAnonymousUser($user_data) { // {{{
        return !empty($user_data['is_anonymous_customer']);
    } //}}}
}

class XCUserActivity {
    private static $default_activity = 'N'; // Use 'N' to avoid C rows in xcart_customer_enabled_activities table
    private static $applicable_usertypes = array('A', 'P', 'B');

    public static function isActivityApplicable($usertype = '') { // {{{
        return in_array($usertype, self::$applicable_usertypes);
    } // }}}

    public static function isActivatedUser($userid) { // {{{
        global $sql_tbl;
        return func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[customer_enabled_activities] WHERE userid = " . intval($userid)) > 0;
    } // }}}

    public static function getCharActivity($id, $usertype = '') { // {{{
        global $sql_tbl;

        if (self::isActivityApplicable($usertype)) {
            $activity = func_query_first_cell("SELECT IF(((SELECT userid FROM $sql_tbl[customer_enabled_activities] WHERE userid = '$id') IS NULL),'N','Y')");
        } else {
    ...
    public static function getJoinActiveUsers($on_condition_id = '') { // {{{
        global $sql_tbl;

        if (empty($on_condition_id)) {
            $on_condition_id = "$sql_tbl[customers].id";
        }

        return "INNER JOIN $sql_tbl[customer_enabled_activities] ON $sql_tbl[customer_enabled_activities].userid=$on_condition_id";
    } // }}}

    public static function updateUser($id, $activity = '') { // {{{
        global $sql_tbl;
        $activity = empty($activity) ? self::$default_activity : $activity;

        if ($activity == 'Y')
            db_query("REPLACE INTO $sql_tbl[customer_enabled_activities] SET userid=" . intval($id));
        else
            db_query("DELETE FROM $sql_tbl[customer_enabled_activities] WHERE userid=" . intval($id));

        return true;
    } // }}}

    public static function updateUsers($activity, $customer_condition = '') { // {{{
        global $sql_tbl;
        $activity = empty($activity) ? self::$default_activity : $activity;
        if (!empty($customer_condition)) {
            $sql_condN = "WHERE userid IN (SELECT id FROM $sql_tbl[customers] $customer_condition)";
        } else {
            $sql_condN = '';
        }

        if ($activity == 'Y') {
            db_query("REPLACE INTO $sql_tbl[customer_enabled_activities] SELECT id FROM $sql_tbl[customers] $customer_condition AND usertype IN ('" . implode("', '", self::$applicable_usertypes) . "')");
        } else {
            db_query("DELETE FROM $sql_tbl[customer_enabled_activities] $sql_condN");
        }
...
class XCUserAddressBook {
    ...
    public static function getDataForMail($address_book, $ship2diff, $existing_address, $new_address) { // {{{
        ...
        if (count($res) == 1) {
            $res['S'] = $res['B'] = func_prepare_address($res['B']);
        } else {
            foreach ($res as $type => $address) {
                $res[$type] = func_prepare_address($address);
            }
        }
...
{
    global $files_dir_name, $single_mode, $sql_tbl, $config;
    ...
    if (
        ...
        $users_count = func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[customers] " . XCUserActivity::getJoinActiveUsers("$sql_tbl[customers].id") . " LEFT JOIN $sql_tbl[memberships] ON $sql_tbl[customers].membershipid = $sql_tbl[memberships].membershipid WHERE $sql_tbl[customers].usertype='$usertype' AND $flag_condition AND $sql_tbl[customers].status = 'Y'");
    ...
    if (
        ...
        x_load('backoffice');//For XCZOnes and Func_get_files_location
        if ($zones_to_delete) {
        ...
        if (!empty($config['shop_status'][XCZones::HAS_ADDRESS_MASK])) {
            XCZones::updateAddressFlag();
        }
    ...
    if (in_array($usertype, array('C', 'B'))) {
        db_query("DELETE FROM $sql_tbl[customer_saved_carts] WHERE userid='$user'");

        if (func_is_defined_module_sql_tbl('XPayments_Connector', 'xpc_saved_cards')) {
            db_query("DELETE FROM $sql_tbl[xpc_saved_cards] WHERE userid = '$user'");
            db_query("DELETE FROM $sql_tbl[xpc_saved_cards_migrated] WHERE userid = '$user'");
        }

        if (func_is_defined_module_sql_tbl('EU_Cookie_Law', 'customer_eu_cookie_accesses')) {
            db_query("DELETE FROM $sql_tbl[customer_eu_cookie_accesses] WHERE userid = '$user'");
        }
    }
    db_query("DELETE FROM $sql_tbl[customer_enabled_activities] WHERE userid='$user'");
    ...
    if (func_is_defined_module_sql_tbl('PayPal_Login', 'lwpp')) {
        db_query("DELETE FROM $sql_tbl[lwpp] WHERE userid = '$user'");
    }
...
function func_get_next_providers($exclude)
{
    ...
    $list = func_query("SELECT * FROM $sql_tbl[customers] " . XCUserActivity::getJoinActiveUsers("$sql_tbl[customers].id") . " WHERE id $where AND usertype = 'P' AND status = 'Y'");
...
function func_userinfo($user, $usertype = '', $need_password = false, $need_cc = '', $profile_area = NULL, $merge_cart_info = true, $use_cache = USE_CACHE)
{
   ...
   if ($use_cache == USE_CACHE) {
    ...
    if (
        ...
        $_seller_address['need_dhl_info'] = (func_query_first_cell("SELECT active FROM $sql_tbl[shipping] WHERE active='Y' AND code='DHL'") == 'Y')
    ...
    if (XCUserActivity::isActivityApplicable($userinfo['usertype'])) {
        $userinfo['activity'] = XCUserActivity::getCharActivity($user, $usertype);
    }
    ...
    if ($userinfo['usertype'] == 'C') {
        $userinfo['personal_firstname'] = isset($userinfo['firstname']) ? $userinfo['firstname'] : '';
        $userinfo['personal_lastname'] = isset($userinfo['lastname']) ? $userinfo['lastname'] : '';
    }
        ...
        if (
            ...
            if (!empty($address_def)) {
                ...
                foreach ($address_def as $type => $address) {
                    ...
                    $userinfo = func_userinfo_set_b_s_prefixed_fields($userinfo, $address, $type, $default_address_fields);
                }
            ...
            if (
                ...
                $userinfo = func_userinfo_set_b_s_prefixed_fields($userinfo, $userinfo['address']['S'], 'S', $default_address_fields);
...
function func_change_user_password($xuserid, $new_password, $_old_password = '') { // {{{
    ...
    $updated_data = array(
        'password'              => addslashes(text_crypt(text_hash(stripslashes($new_password)))),
        'change_password'       => 'N',
...
function func_get_shipping_estimate_fields() { // {{{
    global $shipping_estimate_fields;

    if ($shipping_estimate_fields['zipcode']['avail'] == 'Y') {
        $fields = func_get_default_fields(array('H','C'), 'address_book');
        $shipping_estimate_fields['zipcode']['required'] = $fields['zipcode']['required'];
    }

    return $shipping_estimate_fields;
} // }}} func_get_shipping_estimate_fields
...
function func_get_default_fields($fields_area, $page = 'user_profile', $avail_only = false, $quick_hash = false) { // {{{
    ...
    $_config_register_fields = isset($config['User_Profiles']['register_fields'])
            ? $config['User_Profiles']['register_fields']
            : false;
    $_config_address_book_fields = isset($config['User_Profiles']['address_book_fields'])
    ...
    $md5_args = md5(serialize(array(
        ...
        $_config_register_fields,
        $_config_address_book_fields,
...
function func_store_login_action($userid, $usertype = 'C', $action = 'login', $status = 'success')
{
    ...
    if (!mt_rand(0, 600)) {
        ...
        db_query("DELETE FROM $sql_tbl[login_history] WHERE date_time < " . (XC_TIME - $_period));
    }
...
function func_get_userid_by_login($login = '')
{
    ...
    if (!empty($login))
        $id = func_query_first_cell("SELECT id FROM $sql_tbl[customers] WHERE login='" . addslashes($login). "' AND " . XCUserSql::getSqlRegisteredCond());
...
function func_find_similar_emails($usertype = array(), $detailed = true, $limit = 100)
{
    ...
    if (!empty($usertype)) {
        ...
        $usertype_condition = " AND usertype IN ('" . implode("','", $usertype) . "')";
    }
    ...
    $nonuniq_emails = func_query_column("SELECT email FROM $sql_tbl[customers] WHERE " . XCUserSql::getSqlRegisteredCond() . ($usertype_condition ? $usertype_condition : '') . "GROUP BY email, usertype HAVING COUNT(*) > 1 ORDER BY login ASC, last_login DESC $limit_sql");
    ...
    if (!empty($nonuniq_emails) && $detailed) {
        ...
        foreach ($nonuniq_emails as $email) {
            ...
            $accounts[$email] = func_query("SELECT id, firstname, lastname, login, last_login, usertype FROM $sql_tbl[customers] WHERE email='" . addslashes($email). "' AND " . XCUserSql::getSqlRegisteredCond() . $limit_sql2);
    ...
    $error_types = array(
        1  => array(
            ...
            'error'  => defined('IS_ADMIN_USER')
                        ? ($config['email_as_login'] == 'Y' ? 'txt_user_already_exists_in_orders_emails': 'txt_user_already_exists_in_orders')
        ...
        19 => array(
            'fields' => array('email'),
            'error'  => 'txt_err_msg_code_96' // will be overwritten by alt_text
        ),
        20 => array(
            'fields' => array('status'),
            'error'  => 'txt_err_msg_code_96' // will be overwritten by alt_text
        ...
        22 => array(
            ...
            'error'  => 'txt_ups_av_reenter_fields'
        ),
    );
 ...
 * @see    ____func_see____
 */
function func_check_address($data, $fields_area, $is_reg_section = false, $ups_av_validate = 'run') {//{{{
    global $sql_tbl, $default_address_book_fields, $login_type;
    global $config, $states, $countries, $current_area, $active_modules, $REQUEST_METHOD;
    ...
    foreach (
        ...
        if (!isset($default_fields[$v]) || empty($default_fields[$v])) {
    ...
    if (
        ...
        if (
            is_array($states)
            && $display_states
            && !func_check_state($states, stripslashes(empty($data['state']) ? '' : $data['state']), $data['country'])
        ) {
    ...
    if (
        ...
        && $ups_av_validate !== 'skip_ups_av_validate'
    ) {
...
/**
 * This function set userinfo['s_*'] or userinfo['b_*'] fields from $address. Must be called after func_prepare_address()->func_split_by_eol() to insure the (s_)b_address_2 are set correctly
 */
function func_userinfo_set_b_s_prefixed_fields($userinfo, $address, $type, $default_address_fields = null) {
    // Must be called after func_prepare_address() to insure the (s_)b_address_2 are set correctly
    // func_prepare_address() create (s_)b_address_2 fields using func_split_by_eol call

    $_address_line2 = strtolower($type) . '_' . 'address_2';
    if (isset($userinfo[$_address_line2])) {
        unset($userinfo[$_address_line2]);
    }
...
function func_adjust_customer_address($l_cart, $l_userinfo)
{
    ...
    foreach (array('B', 'S') as $prefix) {
        ...
        if (!empty($$fname)) {
            ...
            $l_userinfo = func_userinfo_set_b_s_prefixed_fields($l_userinfo, $addr, $prefix);
        }
    }
...
function func_admin_save_address_book($address_book, $userid, $additional_values_hash = array())
{
    ...
    foreach ($address_book as $addrid => $data) {
        ...
        func_save_address_additional_values(
            !empty($addrid) ? $addrid : $new_addressid,
            !empty($additional_values_hash[$addrid]) ? $additional_values_hash[$addrid] : array()
        );
    }
...
function func_customer_save_address_book_insession($l_cart, $address_book, $ship2diff, $additional_values_hash = array())
{
    ...
    foreach ($address_book as $addrid => $data) {
        ...
        if ($is_anonymous) {
            ...
            $l_cart = func_set_cart_address($l_cart, $addrid, func_stripslashes($data), '', empty($additional_values_hash[$addrid]) ? false : func_stripslashes($additional_values_hash[$addrid]));
        }
 ...
 */
function func_customer_save_address_book_indb($address_book, $ship2diff, $existing_address, $new_address, $additional_values_hash = array(), $l_userid = 0)
{
    ...
    $l_userid = $l_userid ?: $logged_userid;

    $is_address_book_empty = func_is_address_book_empty($l_userid);
    ...
    foreach ($address_book as $addrid => $data) {
        ...
        if (
            ...
            $_res = func_save_address($l_userid, 0, $data);
            $address_book[$addrid]['id'] = $_res['addressid'];
            func_save_address_additional_values($address_book[$addrid]['id'], empty($additional_values_hash[$addrid]) ? array() : $additional_values_hash[$addrid]);
        } elseif (@$existing_address[$addrid] > 0) {
            // Update address book from checkout
            func_save_address($l_userid, $existing_address[$addrid], $data);
            func_save_address_additional_values($existing_address[$addrid], empty($additional_values_hash[$addrid]) ? array() : $additional_values_hash[$addrid]);
        }
    }
...
function func_save_address_additional_values($addressid, $address_additional_values) { //{{{
    ...
    if (!empty($addressid)) {
        // Delete unchecked checkboxes. (They are not passed in $address_additional_values array)
        db_query("DELETE fv.* FROM $sql_tbl[register_field_address_values] AS fv INNER JOIN $sql_tbl[register_fields] AS f ON fv.addressid = '$addressid' AND fv.fieldid=f.fieldid AND f.type='C'");
    }
...
function func_does_customer_have_tax_examption($userinfo)
{
    ...
    // Check selected Tax operation scheme
    switch ($config['Taxes']['tax_operation_scheme']) {

        case XCTaxesDefs::TAX_SCHEME_GENERAL:

            if (!empty($userinfo['tax_exempt']) && $userinfo['tax_exempt'] == 'Y') {
                $result = 'Y';
            }

            break;

        case XCTaxesDefs::TAX_SCHEME_NO_TAXES:
        ...
        case XCTaxesDefs::TAX_SCHEME_NO_TAXES_FOR_VALIDATED:
            if (
                !empty($userinfo['tax_number'])
                && !empty($userinfo['b_country'])
            ) {
                // Load VAT number checker
                require_once $xcart_dir . '/include/classes/class.VatNumberChecker.php';

                if (
                    AXCTaxNumberChecker::VALID_TAX_NUMBER
                    == XCVatNumberChecker::getInstance()->checkVATnumber($userinfo['b_country'], $userinfo['tax_number'])
                )
                {
                    $result = 'Y';
                }
        ...
        case XCTaxesDefs::TAX_SCHEME_NO_TAXES_FOR_VALIDATED_TRANSBORDER:
            if (
                !empty($userinfo['tax_number'])
                && !empty($userinfo['b_country'])
            ) {
                ...
                if (XCVatNumberChecker::getInstance()->isCountryAllowed($config['Company']['location_country'])) {
                    // Shop country is in EU country list
                    $vatToCheck = array (
                        'countryCode' => $userinfo['b_country'],
                        'vatNumber' => $userinfo['tax_number'],
                    );
                    // Get VAT info
                    $vatInfo = XCVatNumberChecker::getInstance()->hasCountryCodeInVAT($vatToCheck['vatNumber']);
                    // Check if VAT has country code in it
                    if (!empty($vatInfo)) {
                        // Use parsed vat info
                        $vatToCheck = $vatInfo;
                    }
                    // Check country code and validity
                    if (
                        $config['Company']['location_country'] != $vatToCheck['countryCode']
                        && AXCTaxNumberChecker::VALID_TAX_NUMBER
                           == XCVatNumberChecker::getInstance()->checkVATnumber($vatToCheck['countryCode'], $vatToCheck['vatNumber'])
                    ) {
                        $result = 'Y';
                    }
                }
