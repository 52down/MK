 ...
 * @version    b57715788319fbdd4db6378ee5717e4952f63919, v81 (xcart_4_7_8), 2017-03-22 11:02:11, process_category.php, aim
...
if ($REQUEST_METHOD == 'POST') {
    ...
    if ($mode == 'apply') {
        ...
        if (!empty($posted_data)) {
            ...
            $has_cats2enabled = XCRangeProductIdsAdmin::hasCatsToBeEnabled($posted_data);

            foreach ($posted_data as $k => $v) {
                $_category_id = intval($k);
                ...
                func_array2update('categories', $query_data, "categoryid='".$_category_id."'");

                func_reflect_category_availability_changes($_category_id);
            }
            XCProducts_CategoriesChange::repairIntegrity(array_keys($posted_data));

            if (!empty($has_cats2enabled)) {
                // Clear range products cache for cats with products
...
$confirmed = empty($confirmed) ? '' : $confirmed;
